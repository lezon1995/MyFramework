...
public class ObsSystem
{
    ...
    {
        ResourceManager.loadAssetsFromUrl(mURL + remotePath, bytes =>
        {
            callback?.Invoke(bytes, bytes?.Length ?? 0);
        });
    }
    ...
    {
        foreach (var file in getFileListInternal(fileName))
        {
        ...
        while (reader.Read())
        {
            ...
            if (reader.Name == "Contents")
            {
                ...
                while (reader.Read())
                {
                    ...
                    {
                        long.TryParse(reader.Value, out info.size);
                        // 完成一个文件信息的解析
                        fileList.Add(info);
                        break;
                    }
