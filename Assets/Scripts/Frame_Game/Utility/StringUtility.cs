...
{
    private static char[] hexLowerChar = { 'a', 'b', 'c', 'd', 'e', 'f' }; // 十六进制中的小写字母
    private static string[] floatConvertPrecision = { "f0", "f1", "f2", "f3", "f4", "f5", "f6", "f7" }; // 浮点数转换时精度
    ...
    public static string FToS(float value, int precision = 4)
    {
        ...
        return value.ToString(floatConvertPrecision[precision]);
    }
    ...
    protected static void byteToHEXString(StringBuilder builder, byte value)
    {
        ...
        {
            builder.Append(hexLowerChar[high - 10]);
        }
        ...
        {
            builder.Append(hexLowerChar[low - 10]);
        }
    }
}