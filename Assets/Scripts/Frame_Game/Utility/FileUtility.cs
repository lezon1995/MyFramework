...
public class FileUtility
{
    ...
    {
        openFileAsync(fileName, errorIfNull, bytes =>
        {
    ...
    public static void openFileAsync(string fileName, bool errorIfNull, BytesCallback callback)
    {
        if (!isWebGL() && !isFileExist(fileName))
        {
            if (errorIfNull)
            {
                logErrorBase("file not exist:" + fileName);
            }
    ...
    {
        openFileAsync(source, true, bytes =>
        {
            if (bytes != null)
            {
        ...
        try
        {
            ...
            else
            {
                ...
                if (recursive)
                {
                    ...
                    {
                        findFilesInternal(dir, fileList, patterns, true);
                    }
                }
            }
        }
    ...
    public static void generateMD5List(List<string> fileNameList, StringListCallback callback)
    {
        ...
        else
        {
            if (isIOS())
            {
                callback(generateMD5ListInternalIOS(fileNameList));
            }
            else
            {
    ...
    public static void checkNeedDownloadFile(List<string> modifyList, Dictionary<string, GameFileInfo> streamingInfoList, Dictionary<string, GameFileInfo> persistentInfoList, Dictionary<string, GameFileInfo> remoteInfoList, List<string> ignorePathList)
    {
        ...
        foreach (GameFileInfo remoteInfo in remoteInfoList.Values)
        {
            ...
            if (ignorePathList != null)
            {
                ...
                {
                    if (remoteInfo.name.StartsWith(ignorePath.ToLower()))
                    {
                        isIgnoreFile = true;
                        break;
                    }
            ...
            GameFileInfo streamInfo = streamingInfoList.get(remoteInfo.name);
            GameFileInfo persistentInfo = persistentInfoList.get(remoteInfo.name);
            // streaming或者persistent中的大小和md5都与远端一致,则不需要下载
            if ((persistentInfo != null && persistentInfo.size == remoteInfo.size && persistentInfo.md5 == remoteInfo.md5) ||
                (streamInfo != null && streamInfo.size == remoteInfo.size && streamInfo.md5 == remoteInfo.md5))
            {
                continue;
            }

            modifyList.Add(remoteInfo.name);
            // 打印出需要下载的原因,用于调试
            if (streamInfo == null && persistentInfo == null)
            {
                logBase("文件在本地不存在:" + remoteInfo.name);
            }
            else if (streamInfo == null)
            {
                if (persistentInfo.size != remoteInfo.size)
                {
                    logBase("文件:" + remoteInfo.name + "与远端大小不一致,远端:" + remoteInfo.size + ", persistent本地:" + persistentInfo.size);
                }

                if (persistentInfo.md5 != remoteInfo.md5)
                {
                    logBase("文件:" + remoteInfo.name + "与远端MD5不一致,远端:" + remoteInfo.md5 + ", persistent本地:" + persistentInfo.md5);
                }
            }
            else if (persistentInfo == null)
            {
                if (streamInfo.size != remoteInfo.size)
                {
                    logBase("文件:" + remoteInfo.name + "与远端大小不一致,远端:" + remoteInfo.size + ", StreamingAssets本地:" + streamInfo.size);
                }

                if (streamInfo.md5 != remoteInfo.md5)
                {
                    logBase("文件:" + remoteInfo.name + "与远端MD5不一致,远端:" + remoteInfo.md5 + ", StreamingAssets本地:" + streamInfo.md5);
                }
            }
            else
            {
