...
public class ResLocalizationRuntimeText : MonoBehaviour
{
    ...
    public void OnValidate()
    {
        if (!Application.isPlaying)
        {
            ...
            // 中文繁体
            int chineseTradiIndex = mLanguageOriginFontSize.FindIndex((item) => item.mLanguage == LANGUAGE_CHINESE_TRADITIONAL);
            ...
            // 英文
            if (mLanguageOriginFontSize.FindIndex((item) => item.mLanguage == LANGUAGE_ENGLISH) < 0)
            {
