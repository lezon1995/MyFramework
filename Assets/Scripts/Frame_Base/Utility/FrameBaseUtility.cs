...
{
    static bool mShowMessageBox = true; // 是否显示报错提示框,用来判断提示框显示次数
    static int mMainThreadID; // 主线程ID
    ...
    public static VERSION_COMPARE compareVersion3(string remote, string local, out VERSION_COMPARE lowerVersion, out VERSION_COMPARE higherVersion)
    {
        ...
        long[] source = { long.Parse(sourceFormatStr[0]), long.Parse(sourceFormatStr[1]), long.Parse(sourceFormatStr[2]) };
        long[] target = { long.Parse(targetFormatStr[0]), long.Parse(targetFormatStr[1]), long.Parse(targetFormatStr[2]) };
