    ...
    public class MMGizmoEditor : Editor
    {
        ...
        static void Initialization(MMGizmo mmGizmo)
        {
            ...
            mmGizmo._textGUIStyle = new GUIStyle
            {
                normal =
                {
                    textColor = mmGizmo.TextColor,
                    background = MMGUI.MakeTex(600, 100, mmGizmo.TextBackgroundColor)
                },
                fontSize = mmGizmo.TextSize,
                fontStyle = mmGizmo.TextFontStyle,
                padding = new RectOffset((int)mmGizmo.TextPadding.x, (int)mmGizmo.TextPadding.y, (int)mmGizmo.TextPadding.z, (int)mmGizmo.TextPadding.w)
            };
        ...
        static void DrawColliderGizmo(MMGizmo mmGizmo)
        {
            ...
            if (mmGizmo._boxCollider2DNotNull)
            {
                ...
                {
                    x = mmGizmo._boxCollider2D.size.x,
                    y = mmGizmo._boxCollider2D.size.y,
                    z = 0.1f
                };
                if (mmGizmo.ColliderRenderType == MMGizmo.ColliderRenderTypes.Full)
        ...
        static void DrawPositionGizmo(MMGizmo mmGizmo)
        {
            switch (mmGizmo.PositionMode)
            {
                ...
                case MMGizmo.PositionModes.Arrows:
                    ...
                    Handles.ArrowHandleCap(0, ComputeGizmoPosition(mmGizmo, mmGizmo.transform.position, false), Quaternion.LookRotation(mmGizmo.transform.right, mmGizmo.transform.up), mmGizmo.PositionSize, EventType.Repaint);
                    Handles.color = Handles.yAxisColor;
                    Handles.ArrowHandleCap(0, ComputeGizmoPosition(mmGizmo, mmGizmo.transform.position, false), Quaternion.LookRotation(mmGizmo.transform.up, mmGizmo.transform.up), mmGizmo.PositionSize, EventType.Repaint);
                    Handles.color = Handles.zAxisColor;
                    Handles.ArrowHandleCap(0, ComputeGizmoPosition(mmGizmo, mmGizmo.transform.position, false), Quaternion.LookRotation(mmGizmo.transform.forward, mmGizmo.transform.up), mmGizmo.PositionSize, EventType.Repaint);
                    break;
        ...
        static Vector3 ComputeGizmoPosition(MMGizmo mmGizmo, Vector3 position, bool relativeLock = true)
        {
            ...
            if (mmGizmo.LockX || mmGizmo.LockY || mmGizmo.LockZ)
            {
                ...
                if (mmGizmo.LockX)
                {
                    mmGizmoNewPosition.x = relativeLock ? -mmGizmo.transform.position.x + mmGizmo.LockedX : mmGizmo.LockedX;
                }

                if (mmGizmo.LockY)
                {
                    mmGizmoNewPosition.y = relativeLock ? -mmGizmo.transform.position.y + mmGizmo.LockedY : mmGizmo.LockedY;
                }

                if (mmGizmo.LockZ)
                {
                    mmGizmoNewPosition.z = relativeLock ? -mmGizmo.transform.position.z + mmGizmo.LockedZ : mmGizmo.LockedZ;
                }
