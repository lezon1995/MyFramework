...
namespace MoreMountains.Tools
{
    ...
    {
        const float HORIZONTAL_GAP = 5;
        const float VERTICAL_GAP = 5;
        ...
        void OnAddElement(Rect rect, ReorderableList list)
        {
        ...
        protected virtual void DrawContextMenu(SerializedProperty variables, int index)
        {
            ...
            foreach (VariableType variableType in System.Enum.GetValues(typeof(VariableType)))
            {
                ...
                menu.AddItem(new GUIContent(variableType.ToString()), false, context => { AddVariable(variables, index, type); }, null);
            }
