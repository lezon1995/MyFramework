...
namespace MoreMountains.Feedbacks
{
    ...
    public sealed class MMF_FeedbackInspector
    {
        ...
        const string _channelModeFieldName = "ChannelMode";
        const string _channelDefinitionFieldName = "MMChannelDefinition";
        const string _automatedTargetAcquisitionName = "AutomatedTargetAcquisition";
        ...
        Dictionary<string, MMFConditionAttribute> _conditionDictionary = new Dictionary<string, MMFConditionAttribute>();
        Dictionary<string, MMFEnumConditionAttribute> _enumConditionDictionary = new Dictionary<string, MMFEnumConditionAttribute>();
        MMFConditionAttribute _conditionAttributeStore;
        MMFEnumConditionAttribute _enumConditionAttributeStore;

        public void Initialization(SerializedProperty currentProperty, MMF_Feedback feedback, bool expandGroupInspectors)
        {
            ...
            for (int i = 0; i < fieldInfoLength; i++)
            {
                ...
                if (group == null)
                {
                    if (previousGroupAttribute is { GroupAllFieldsUntilNextGroupAttribute: true })
                    {
        ...
        bool ShouldSkipGroup(string groupName)
        {
            bool skip = false;
        ...
        void SearchForConditions(FieldInfo fieldInfo)
        {
        ...
        bool DrawBase(SerializedProperty currentProperty, MMF_Feedback feedback)
        {
        ...
        void DrawContainer(MMF_Feedback feedback)
        {
        ...
        void DrawContents(MMF_Feedback feedback)
        {
            if (PropertiesList.Count == 0)
            {
                return;
        ...
        Rect _leftBorderRect;
        Rect _setupRect;
        Rect _verticalGroup;
        Rect _widthRect;
        GUIContent _groupTitle = new GUIContent();

        void DrawGroup(MMFInspectorGroupData groupData, MMF_Feedback feedback)
        {
            ...
            if (groupData.GroupIsOpen)
            {
                ...
                {
                    var index = i;
                    DrawVerticalLayout(() => DrawChild(index, feedback), MMF_FeedbackInspectorStyle.BoxChildStyle);
                }
            }
                ...
                switch (currentProperty.type)
                {
                    ...
                    case _customTweenTypePropertyName:
                        ...
                        if (_enumConditionDictionary.TryGetValue(currentProperty.name, out _enumConditionAttribute))
                        {
                            ...
                            if (sourcePropertyValue is { propertyType: SerializedPropertyType.Enum })
                            {
