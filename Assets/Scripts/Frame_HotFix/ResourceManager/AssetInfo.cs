...
{
    protected List<AssetLoadDoneCallback> callbacks = new(); // 异步加载回调列表
    protected List<string> loadPaths = new(); // 加载资源时使用的路径
    protected UObject[] subAssets; // 资源数组,数组第一个元素为主资源,后面的是子资源
    protected AssetBundleInfo parentAssetBundle; // 资源所属的AssetBundle
    protected string assetName; // 资源文件名,带相对于StreamingAssets的相对路径,带后缀
    protected LOAD_STATE loadState = LOAD_STATE.NONE; // 加载状态

    public void setAssetBundleInfo(AssetBundleInfo parent)
    {
        parentAssetBundle = parent;
    }
    ...
    public AssetBundleInfo getAssetBundle()
    {
    ...
    public void callbackAll()
    {
        ...
        callbacks.move(this.callbacks);
        paths.move(loadPaths);
        int callbackCount = callbacks.Count;
        for (int i = 0; i < callbackCount; ++i)
        {
            callbacks[i](subAssets.get(0), subAssets, null, paths[i]);
        }
    }
    ...
    protected void doLoadAssets()
    {
        ...
        subAssets = parentAssetBundle.getAssetBundle().LoadAssetWithSubAssets(P_GAME_RESOURCES_PATH + assetName);
        loadState = LOAD_STATE.LOADED;
    }
}