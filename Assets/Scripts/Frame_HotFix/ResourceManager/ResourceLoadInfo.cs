...
{
    public List<AssetLoadDoneCallback> callbacks = new(); // 回调列表
    public List<string> loadPaths = new(); // 用于回调传参的加载路径列表,实际上里面都是mResourceName
    protected UObject[] subAssets; // 子物体列表,比如图集中的所有Sprite
    protected UObject asset; // 资源物体
    protected string path; // 加载路径,也就是mResourceName中的路径,不带文件名
    ...
    public void callbackAll()
    {
        ...
        tempCallbackList.move(callbacks);
        tempLoadPath.move(loadPaths);
