...
public class StateManager : FrameSystem
{
    ...
    {
        return mStateTypeList.get(id);
    }

    public Type getParamType(int id)
    {
        return mStateParamList.get(id);
    }

    public Dictionary<Type, StateGroup> getGroupStateList()
    {
        return mGroupStateList;
    }

    public Dictionary<Type, List<Type>> getStateGroupList()
    {
        return mStateGroupList;
    }

    public List<Type> getGroupList(Type stateType)
    {
        return mStateGroupList.get(stateType);
    }

    public StateGroup getGroup(Type groupType)
    {
        return mGroupStateList.get(groupType);
    }

    public Type getGroupMutex(GROUP_MUTEX mutex)
    {
        return mGroupMutexList.get(mutex);
    }
