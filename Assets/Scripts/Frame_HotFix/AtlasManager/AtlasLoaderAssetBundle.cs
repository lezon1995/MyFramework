...
public class AtlasLoaderAssetBundle : AtlasLoaderBase
{
	...
	{
		res.unload(ref atlas.mMainAsset, showError); 
	}
	protected override CustomAsyncOperation baseLoadAtlasAsync(string atlasName, AssetLoadDoneCallback doneCallback)
	{
		...
		{
			return res.loadGameResourceAsync<Sprite>(atlasName, doneCallback);
		}
		else
		{
			return res.loadGameResourceAsync<SpriteAtlas>(atlasName, doneCallback);
		}
	}
	protected override UObject[] baseLoadSubResource(string atlasName, out UObject mainAsset, bool errorIfNull) 
	{
		if (atlasName.endWith(".png"))
		{
			return res.loadSubGameResource<Sprite>(atlasName, out mainAsset, errorIfNull);
		}
		else
		{
			return res.loadSubGameResource<SpriteAtlas>(atlasName, out mainAsset, errorIfNull);
		}
	}
