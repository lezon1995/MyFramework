...
{
    protected Dictionary<KeyCode, string> keys = new(); // 快捷键名字列表
    protected Dictionary<int, KeyMapping> keyMappings = new(); // 按键映射表,key是自定义的一个整数,用于当作具体功能的枚举,value是对应的按键
    ...
    public Dictionary<int, KeyMapping> getKeyMappingList()
    {
        return keyMappings;
    }

    public string getKeyMappingName(int keyMapping)
    {
        return getKeyListName(getKeyMapping(keyMapping));
    }
    ...
    public string getKeyListName(KeyCode key)
    {
        return key != KeyCode.None ? getKeyName(key) : "";
    }

    public bool isMappingKeyCurrentDown(int mapping)
    {
        return mInputSystem.isKeyCurrentDown(getKeyMapping(mapping), FOCUS_MASK.SCENE);
    }

    public bool isMappingKeyDown(int mapping)
    {
        return mInputSystem.isKeyDown(getKeyMapping(mapping), FOCUS_MASK.SCENE);
    }

    public string getKeyMappingActionName(int mapping)
    {
        return keyMappings.TryGetValue(mapping, out var info) ? info.mappingName : "";
    }

    public KeyCode getKeyMapping(int mapping)
    {
        return keyMappings.TryGetValue(mapping, out var info) ? info.key : KeyCode.None;
    }

    public KeyCode getDefaultMappingKey(int mapping)
    {
        return keyMappings.TryGetValue(mapping, out var key) ? key.defaultKey : KeyCode.None;
    }

    public void setDefaultKeyMapping(int mapping, KeyCode key)
    {
        keyMappings.get(mapping).defaultKey = key;
    }
    ...
    public bool setKeyMapping(int mappingID, KeyCode key, string actionName = null)
    {
        ...
        {
            // 需要检测是否与当前按键映射冲突
            foreach (var item in keyMappings.Values)
            {
                if (item.key == key && item.mappingId != mappingID)
                {
                    return false;
                }
            }
        ...
        // 更新或者添加
        keyMappings.getOrAddClass(mappingID, out var curMapping);
        if (!actionName.isEmpty())
        {
            curMapping.mappingName = actionName;
        }

        curMapping.mappingId = mappingID;
        curMapping.key = key;
        return true;
    }
