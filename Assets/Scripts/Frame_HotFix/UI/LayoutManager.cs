...
public class LayoutManager : FrameSystem
{
    ...
    public void setUseAnchor(bool useAnchor)
    {
        mUseAnchor = useAnchor;
    }
    ...
    {
        return mLayoutTypeToPath.get(type);
    }

    public Type getLayoutTypeByPath(string path)
    {
        return mLayoutPathToType.get(path);
    }

    public GameLayout getLayout(Type type)
    {
        return mLayoutList.get(type);
    }
    ...
    {
        if (mLayoutList.tryGetValue(info.type, out GameLayout existLayout))
        {
            return existLayout;
        }
        ...
        if (mLayoutRegisteList.get(info.type).inResource)
        {
    ...
    public void createLayoutAsync(LayoutInfo info, GameLayoutCallback callback)
    {
        ...
        info.name = getFileNameNoSuffixNoDir(path);
        string pathUnderResource = R_UI_PREFAB_PATH + path + ".prefab";
        mLayoutAsyncList.Add(info.name, info);
        if (mLayoutRegisteList.get(info.type).inResource)
        {
            mResourceManager.loadInResourceAsync(pathUnderResource, (GameObject asset) =>
            {
                if (mLayoutAsyncList.Remove(asset.name, out LayoutInfo layoutInfo))
                {
    ...
    {
        return mLayoutTypeToPath.Count;
    }
    ...
    public void unloadAllPartLayout()
    {
        ...
        {
            if (mLayoutRegisteList.get(type).lifeCycle == LAYOUT_LIFE_CYCLE.PART_USE)
            {
                LT.UNLOAD(type);
            }
        }
    ...
    {
        mLayoutRegisteList.get(layout.getType()).callback?.Invoke(layout.getScript());
    }

    // 方便调用的布局注册函数
    public static void registeLayoutResPart<T>(Action<T> callback = null) where T : LayoutScript
    {
        registeLayout<T>(true, LAYOUT_LIFE_CYCLE.PART_USE, (script) =>
        {
            callback?.Invoke(script as T);
        });
    }

    public static void registeLayoutResAlways<T>(Action<T> callback = null) where T : LayoutScript
    {
        registeLayout<T>(true, LAYOUT_LIFE_CYCLE.PERSIST, (script) =>
        {
            callback?.Invoke(script as T);
        });
    }
    ...
    {
        mLayoutManager.registeLayout(typeof(T), name, inResource, lifeCycle, (script) =>
        {
            callback?.Invoke(script as T);
        });
    }
    ...
    protected GameLayout newLayout(LayoutInfo info, GameObject prefab)
    {
        ...
        layout.setOrderType(info.layoutOrder);
        layout.setRenderOrder(generateRenderOrder(layout, info.renderOrder, info.layoutOrder));
        layout.setInResources(mLayoutRegisteList.get(info.type).inResource);
        layout.init();
        ...
        mLayoutList.add(info.type, layout);
        return layout;
    }
}