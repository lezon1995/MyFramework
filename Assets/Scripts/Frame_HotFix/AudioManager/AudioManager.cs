...
public class AudioManager : FrameSystem
{
    ...
    public override void update(float dt)
    {
        ...
        {
            // 本身就小于0表示不自动销毁
            if (tickTimerOnce(ref item.mRemainTime, dt))
            {
                unusedHelper(item);
            }
        }
    ...
    public Dictionary<string, AudioInfo> getAudioList()
    {
        return mAudioList;
    }

    public AudioHelper getMusicHelper()
    {
        return mMusicHelper;
    }
    ...
    public AudioHelper getAudioHelper(float audioTime)
    {
        return getOneUnusedHelper(audioTime);
    }
    ...
    public void destroyAudioHelper(AudioHelper helper)
    {
        unusedHelper(helper);
    }

    public void setMaxAudioCount(int maxCount)
    {
        mMaxAudioCount = maxCount;
    }
    ...
    {
        return getAudioLength(mAudioManager.getAudioName(sound));
    }
    ...
    {
        return mLoadedCount == mAudioList.Count;
    }

    public float getLoadedPercent()
    {
        return divide(mLoadedCount, mAudioList.Count);
    }

    public string getAudioName(int soundDefine)
    {
        return mSoundDefineMap.get(soundDefine);
    }
