...
public class ArrayPool : FrameSystem
{
    ...
    public Dictionary<Type, Dictionary<int, HashSet<Array>>> getPersistentInusedList()
    {
        return mPersistentInuseList;
    }

    public Dictionary<Type, Dictionary<int, HashSet<Array>>> getInusedList()
    {
        return mInusedList;
    }

    public Dictionary<Type, Dictionary<int, Queue<Array>>> getUnusedList()
    {
        return mUnusedList;
    }
    ...
    public T[] newArray<T>(int size, bool onlyOnce = true)
    {
        ...
        if (isEditor())
        {
            ...
            {
                logNoLock(typeof(T) + "[" + size + "]" + "数量已经达到了" + mCreatedCount + "个");
            }
        }
