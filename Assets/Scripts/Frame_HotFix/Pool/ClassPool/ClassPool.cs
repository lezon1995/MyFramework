...
{
    protected Dictionary<Type, HashSet<ClassObject>> persistentUsingList = new(); // 持久使用的对象列表,为了提高运行时效率,仅在编辑器下使用
    protected Dictionary<Type, HashSet<ClassObject>> usedList = new(); // 仅这一帧使用的对象列表,为了提高运行时效率,仅在编辑器下使用
    protected Dictionary<Type, Queue<ClassObject>> unusedList = new(); // 未使用的对象列表
    protected Dictionary<ClassObject, string> objectStack = new(); // 存储堆栈信息的列表
    protected static long assignedIDSeed; // 分配ID,用于标识每一个分配出去的对象
    ...
    public override void init()
    {
        ...
        if (isEditor())
        {
            go.AddComponent<ClassPoolDebug>();
        }
    }
    ...
    public override void update(float dt)
    {
        ...
        if (isEditor())
        {
            foreach (var item in usedList.Values)
            {
                foreach (ClassObject itemList in item)
                {
                    string stack = objectStack.get(itemList);
                    if (stack.isEmpty())
                    {
    ...
    {
        foreach (var item in unusedList.Values)
        {
            item.Clear();
        }
    }

    public Dictionary<Type, HashSet<ClassObject>> getPersistentInusedList()
    {
        return persistentUsingList;
    }

    public Dictionary<Type, HashSet<ClassObject>> getInusedList()
    {
        return usedList;
    }

    public Dictionary<Type, Queue<ClassObject>> getUnusedList()
    {
        return unusedList;
    }
    ...
    public ClassObject newClass(Type type, bool onlyOnce)
    {
        ...
        ClassObject obj;
        // 先从未使用的列表中查找是否有可用的对象
        if (unusedList.TryGetValue(type, out var classList) && classList.Count > 0)
        {
        ...
        obj.setAssignID(++assignedIDSeed);
        obj.setDestroy(false);

        if (isEditor())
        {
            // 添加到已使用列表
            var inuseList = onlyOnce ? usedList : persistentUsingList;
            ...
            objectStack.Add(obj, GameEntry.getInstance().frameworkParam.enablePoolStackTrace ? getStackTrace() : EMPTY);

            if (isNew)
            {
                int totalCount = 0;
                totalCount += usedList.get(type)?.Count ?? 0;
                totalCount += persistentUsingList.get(type)?.Count ?? 0;
    ...
    public T newClass<T>(bool onlyOnce) where T : ClassObject => newClass(typeof(T), onlyOnce) as T;
    ...
    public void destroyClass<T>(ref T classObject) where T : ClassObject
    {
        ...
        // 加入未使用列表
        var objList = unusedList.getOrAddNew(type);
        if (isEditor())
        {
            objectStack.Remove(temp);
    ...
    public void destroyClassList<T>(ICollection<T> classObjectList) where T : ClassObject
    {
        ...
        foreach (T classObject in classObjectList)
        {
            ...
            if (isEditor())
            {
                objectStack.Remove(classObject);
    ...
    protected void removeInuse(ClassObject classObject, Type type)
    {
        ...
        if (usedList.TryGetValue(type, out var classList) && classList.Remove(classObject))
        {
            return;
        }

        if (persistentUsingList.TryGetValue(type, out classList) && classList.Remove(classObject))
        {
            return;
        }
