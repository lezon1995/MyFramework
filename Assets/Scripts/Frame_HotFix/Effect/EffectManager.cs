...
public class EffectManager : FrameSystem
{
    ...
    protected SafeList<GameEffect> effectList = new(); // 特效列表
    ...
    public override void init()
    {
        ...
        if (isEditor())
        {
            go.AddComponent<EffectManagerDebug>();
        }
    }
    ...
    public SafeList<GameEffect> getEffectList()
    {
        return effectList;
    }
    ...
    {
        return effectList.add(create(effectObject, effectObject.name, null, 0, active, lifeTime, true, true));
    }
    ...
    {
        return mPrefabPoolManager.createObjectAsync(nameWithPath, tag, moveToHide, true, go =>
        {
    ...
    {
        return mPrefabPoolManager.createObjectAsyncSafe(relatedObject, nameWithPath, tag, moveToHide, true, go =>
        {
            // 这里不能合并成一行,否则callback为null时无法执行创建特效的函数
    ...
    protected GameEffect getEffectFromUnused(GameObject parent, string nameWithPath, Vector3 pos, bool moveToHide, bool active, float lifeTime)
    {
        ...
        return this.effectList.add(effect);
    }
