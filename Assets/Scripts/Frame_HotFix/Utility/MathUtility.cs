...
{
    static List<AStarNode> mTempOpenList = new(); // 避免GC
    static Point[] mTempDirect4 = new Point[4]; // 避免GC
    static Point[] mTempDirect6 = new Point[6]; // 避免GC
    static Point[] mTempDirect8 = new Point[8]; // 避免GC
    static AStarNode[] mTempNodeList; // 避免GC
    static ThreadLock mGreaterPow2Lock = new();
    static int[] mGreaterPow2; // 预先生成的每个数字所对应的第一个比它大的2的n次方的数
    static float[] mSinList; // PI/(2^n)的sin值,其中n是下标
    static float[] mCosList; // PI/(2^n)的cos值,其中n是下标
    const int MAX_FFT_COUNT = 1024 * 8; // 计算频域时数据的最大数量
    static Complex[] mComplexList = new Complex[MAX_FFT_COUNT]; // 避免GC
    ...
    public static long[] POWER_INT_10 = new long[11]
    {
        1L,
        10L,
        100L,
    ...
    public static float[] INVERSE_POWER_INT_10 = new float[7]
    {
        1.0f,
        0.1f,
        0.01f,
    ...
    public static double[] INVERSE_POWER_LLONG_10 = new double[11]
    {
        1.0,
        0.1,
        0.01,
    ...
    {
        return divideInt(index, width);
    }

    public static Vector2Int indexToIntPos(int index, int width)
    {
        return new(index % width, divideInt(index, width));
    }

    public static int intPosToIndex(Vector2Int pos, int width)
    {
        return pos.x + pos.y * width;
    }

    public static int intPosToIndex(int x, int y, int width)
    {
        return x + y * width;
    }
    ...
    public static float MtoKM(float m)
    {
        return m * 0.001f;
    }
    ...
    public static float inversePow10(int pow)
    {
        return INVERSE_POWER_INT_10[pow];
    }

    public static int pow10(int pow)
    {
        return (int)POWER_INT_10[pow];
    }

    public static double inversePow10Long(int pow)
    {
        return INVERSE_POWER_LLONG_10[pow];
    }

    public static long pow10Long(int pow)
    {
        return POWER_LLONG_10[pow];
    }

    public static float pow2(int power)
    {
        return 1 << power;
    }
    ...
    public static int step(float value0, float value1)
    {
        return value1 >= value0 ? 1 : 0;
    }

    // 得到value0除以value1的余数
    public static float fmod(float value0, float value1)
    {
        return value0 - value1 * (int)(divide(value0, value1));
    }
    ...
    public static float crossProduct(Vector2 start, Vector2 end, Vector2 point)
    {
    ...
    public static void randomSelect(int allCount, int selectCount, List<int> selectIndexes)
    {
        ...
        for (int i = 0; i < selectCount; ++i)
        {
            ...
            if (index != i)
            {
                (helpList[index], helpList[i]) = (helpList[i], helpList[index]);
            }
        }
    }
    ...
    // 将向量的X设置为0
    public static Vector3 resetX(Vector3 v)
    {
        return new(0.0f, v.y, v.z);
    }

    // 将向量的Y设置为0
    public static Vector3 resetY(Vector3 v)
    {
        return new(v.x, 0.0f, v.z);
    }

    // 将向量的Z设置为0
    public static Vector3 resetZ(Vector3 v)
    {
        return new(v.x, v.y, 0.0f);
    }

    // 将向量的X替换为指定值
    public static Vector3 replaceX(Vector3 v, float x)
    {
        return new(x, v.y, v.z);
    }

    // 将向量的Y替换为指定值
    public static Vector3 replaceY(Vector3 v, float y)
    {
        return new(v.x, y, v.z);
    }

    // 将向量的Z替换为指定值
    public static Vector3 replaceZ(Vector3 v, float z)
    {
        return new(v.x, v.y, z);
    }

    // vec0的3个分量是否都小于vec1的3个分量
    public static bool isVector3Less(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x < vec1.x && vec0.y < vec1.y && vec0.z < vec1.z;
    }

    // vec0的3个分量是否都大于vec1的3个分量
    public static bool isVector3Greater(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x > vec1.x && vec0.y > vec1.y && vec0.z > vec1.z;
    }

    // vec0的2个分量是否都小于vec1的2个分量
    public static bool isVector2Less(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x < vec1.x && vec0.y < vec1.y;
    }

    // vec0的2个分量是否都大于vec1的2个分量
    public static bool isVector2Greater(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x > vec1.x && vec0.y > vec1.y;
    }
    ...
    public static bool isQuaternionEqual(Quaternion q0, Quaternion q1, float precision = 0.0001f)
    {
        return isFloatEqual(q0.x, q1.x, precision) &&
               isFloatEqual(q0.y, q1.y, precision) &&
               isFloatEqual(q0.z, q1.z, precision) &&
               isFloatEqual(q0.w, q1.w, precision);
    }
    ...
    public static float getLengthIgnoreY(Vector3 vec)
    {
    ...
    public static float getSquaredLengthIgnoreY(Vector3 vec)
    {
    ...
    public static bool lengthLess(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y < vec1.x * vec1.x + vec1.y * vec1.y;
    }

    public static bool lengthLess(Vector2 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y < length * length;
    }

    public static bool lengthLess(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y + vec0.z * vec0.z < vec1.x * vec1.x + vec1.y * vec1.y + vec1.z * vec1.z;
    }

    public static bool lengthLess(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z < length * length;
    }

    public static bool lengthLessIgnoreY(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.z * vec.z < length * length;
    }

    public static bool lengthLess(Vector4 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z + vec.w * vec.w < length * length;
    }

    public static bool lengthLessEqual(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y <= vec1.x * vec1.x + vec1.y * vec1.y;
    }

    public static bool lengthLessEqual(Vector2 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y <= length * length;
    }

    public static bool lengthLessEqual(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y + vec0.z * vec0.z <= vec1.x * vec1.x + vec1.y * vec1.y + vec1.z * vec1.z;
    }

    public static bool lengthLessEqual(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z <= length * length;
    }

    public static bool lengthLessEqualIgnoreY(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.z * vec.z <= length * length;
    }

    public static bool lengthGreater(Vector2 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y > length * length;
    }

    public static bool lengthGreater(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y > vec1.x * vec1.x + vec1.y * vec1.y;
    }

    public static bool lengthGreater(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z > length * length;
    }

    public static bool lengthGreaterIgnoreY(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.z * vec.z > length * length;
    }

    public static bool lengthGreater(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y + vec0.z * vec0.z > vec1.x * vec1.x + vec1.y * vec1.y + vec1.z * vec1.z;
    }

    public static bool lengthGreaterEqual(Vector2 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y >= length * length;
    }

    public static bool lengthGreaterEqual(Vector2 vec0, Vector2 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y >= vec1.x * vec1.x + vec1.y * vec1.y;
    }

    public static bool lengthGreaterEqual(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.y * vec.y + vec.z * vec.z >= length * length;
    }

    public static bool lengthGreaterEqual(Vector3 vec0, Vector3 vec1)
    {
        return vec0.x * vec0.x + vec0.y * vec0.y + vec0.z * vec0.z >= vec1.x * vec1.x + vec1.y * vec1.y + vec1.z * vec1.z;
    }

    public static bool lengthGreaterEqualIgnoreY(Vector3 vec, float length)
    {
        return vec.x * vec.x + vec.z * vec.z >= length * length;
    }
    ...
    public static void toRadian(ref float degree)
    {
    ...
    public static void toRadian(ref Vector3 degree)
    {
        degree *= Mathf.Deg2Rad;
    }
    ...
    public static void toDegree(ref float radian)
    {
    ...
    public static void toDegree(ref Vector3 radian)
    {
        radian *= Mathf.Rad2Deg;
    }
    ...
    // 根据公式y = a * x * x + b * x + c
    public static float generateFactorBFromFactorA(float factorA, Vector3 point)
    {
        return divide(point.y - factorA * point.x * point.x, point.x);
    }

    public static float generateFactorA(float factorB, Vector3 point)
    {
        return divide(point.y - factorB * point.x, point.x * point.x);
    }

    public static float generateTopHeight(float factorA, float factorB)
    {
        return -divide(factorB * factorB, 4.0f * factorA);
    }
    ...
    public static Vector3 getMinVector3(Vector3 a, Vector3 b)
    {
        return new(getMin(a.x, b.x), getMin(a.y, b.y), getMin(a.z, b.z));
    }

    public static Vector3 getMaxVector3(Vector3 a, Vector3 b)
    {
        return new(getMax(a.x, b.x), getMax(a.y, b.y), getMax(a.z, b.z));
    }
    ...
    public static uint getMin(uint a, uint b)
    {
        return a < b ? a : b;
    }
    ...
    public static uint getMax(uint a, uint b)
    {
        return a > b ? a : b;
    }
    ...
    public static Vector3 lerpSimple(Vector3 start, Vector3 end, float t)
    {
        return start + (end - start) * t;
    }

    public static Color lerpSimple(Color start, Color end, float t)
    {
        return start + (end - start) * t;
    }
    ...
    public static Vector2 divideVector2(Vector2 v1, Vector2 v2)
    {
        return new(divide(v1.x, v2.x), divide(v1.y, v2.y));
    }
    ...
    public static Vector3 divideVector3(Vector3 v1, Vector3 v2)
    {
        return new(divide(v1.x, v2.x), divide(v1.y, v2.y), divide(v1.z, v2.z));
    }

    public static Vector2 divide(Vector2 v1, float scale)
    {
        return new(divide(v1.x, scale), divide(v1.y, scale));
    }

    public static Vector3 divide(Vector3 v1, float scale)
    {
        return new(divide(v1.x, scale), divide(v1.y, scale), divide(v1.z, scale));
    }
    ...
    public static void adjustRadian180(ref float radian)
    {
        clampCycle(ref radian, -PI_RADIAN, PI_RADIAN, TWO_PI_RADIAN);
    }

    public static float adjustRadian180(float radian)
    {
        return clampCycle(radian, -PI_RADIAN, PI_RADIAN, TWO_PI_RADIAN);
    }
    ...
    public static void adjustAngle180(ref float degree)
    {
        clampCycle(ref degree, -PI_DEGREE, PI_DEGREE, TWO_PI_DEGREE);
    }

    public static float adjustAngle180(float degree)
    {
        return clampCycle(degree, -PI_DEGREE, PI_DEGREE, TWO_PI_DEGREE);
    }
    ...
    public static float adjustRadian360(float radian)
    {
        return clampCycle(radian, 0.0f, TWO_PI_RADIAN, TWO_PI_RADIAN);
    }

    public static void adjustRadian360(ref float radian)
    {
        clampCycle(ref radian, 0.0f, TWO_PI_RADIAN, TWO_PI_RADIAN);
    }
    ...
    public static void adjustAngle360(ref float degree)
    {
        clampCycle(ref degree, 0.0f, TWO_PI_DEGREE, TWO_PI_DEGREE);
    }

    public static float adjustAngle360(float degree)
    {
        return clampCycle(degree, 0.0f, TWO_PI_DEGREE, TWO_PI_DEGREE);
    }
    ...
    public static void rotateVector3(ref Vector3 vec, Matrix4x4 transMat3)
    {
        vec = transMat3 * vec;
    }
    ...
    public static void rotateVector3(ref Vector3 vec, Quaternion transQuat)
    {
        vec = transQuat * vec;
    }
    ...
    // 帧换算成秒
    public static float frameToSecond(float frame)
    {
        return frame * 0.0333f;
    }
    ...
    protected static void postAStar(AStarNode[] nodeList, int endIndex, List<int> foundPath)
    {
        ...
        {
            (foundPath[i], foundPath[count - i - 1]) = (foundPath[count - i - 1], foundPath[i]);
        }
    }
}