...
public class FileUtility
{
        ...
        try
        {
            ...
            else
            {
                foreach (string dir in Directory.GetDirectories(path))
                {
                    ...
                    if (recursive)
                    {
                        findFolders(dir, dirList, excludeList, true);
                    }
                }
            }
        }
    ...
    public static void generateFileMD5(string fileName, StringCallback callback)
    {
        ...
        {
            openFileAsync(fileName, true, fileContent =>
            {
    ...
    public static void checkNeedUploadFile(List<string> modifyList, Dictionary<string, GameFileInfo> remoteList, Dictionary<string, GameFileInfo> localInfoList)
    {
        ...
        foreach (GameFileInfo localInfo in localInfoList.Values)
        {
            ...
            if (!remoteList.TryGetValue(localInfo.name, out GameFileInfo info) ||
                info.size != localInfo.size ||
                info.md5 != localInfo.md5)
            {
                modifyList.Add(localInfo.name);
                if (remoteList.ContainsKey(localInfo.name))
                {
                    if (info.size != localInfo.size)
                    {
                        log("文件:" + localInfo.name + "与本地大小不一致,本地大小:" + localInfo.size + ", 远端大小:" + info.size);
                    }
                    else if (info.md5 != localInfo.md5)
                    {
                        log("文件:" + localInfo.name + "与本地MD5不一致,本地:" + localInfo.md5 + ", 远端:" + info.md5);
                    }
                }
                else
                {
                    log("文件在远端不存在:" + localInfo.name);
                }
            }
        }
    }
    ...
    public static bool checkDiff(Dictionary<string, GameFileInfo> list0, Dictionary<string, GameFileInfo> list1, bool checkMD5)
    {
        ...
        foreach (var item in list0)
        {
                ...
                (checkMD5 && info0.md5 != info1.md5))
            {
                ...
                else if (info0.size != info1.size)
                {
                    logError("file size not same:" + item.Key);
                }
                else if (info0.md5 != info1.md5)
                {
