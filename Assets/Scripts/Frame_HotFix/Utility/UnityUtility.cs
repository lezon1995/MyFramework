...
public class UnityUtility
{
    ...
    protected static Vector2 mScreenScale = new(mScreenSize.x * (1.0f / STANDARD_WIDTH), mScreenSize.y * (1.0f / STANDARD_HEIGHT)); // 当前分辨率相对于标准分辨率的缩放
    ...
    public static void setNormalProperty(GameObject obj, GameObject parent, string name, Vector3 scale, Vector3 rot, Vector3 pos)
    {
        ...
        if (!isVectorEqual(t.localEulerAngles, rot))
        {
            t.localEulerAngles = rot;
        }

        if (!isVectorEqual(t.localScale, scale))
        {
            t.localScale = scale;
        }
    ...
    public static Vector2Int getHardwareScreenSize()
    {
        return mHardwareScreenSize;
    }
    ...
    public static Vector2Int getHalfScreenSize()
    {
    ...
    {
        return getUGUIRoot().getWindowSize();
    }

    // 获取屏幕独立的缩放值
    public static Vector2 getScreenScale()
    {
        return FrameBaseUtility.getScreenScale(mScreenSize);
    }

    // 根据一定规则,获取屏幕的缩放
    public static Vector2 getScreenScale(ASPECT_BASE aspectBase)
    {
