...
public class FrameUtility
{
    ...
    public static GameCamera getMainCamera()
    {
        return mCameraManager?.getMainCamera();
    }

    public static bool isKeyCurrentDown(KeyCode key, FOCUS_MASK mask = FOCUS_MASK.SCENE)
    {
        return mInputSystem.isKeyCurrentDown(key, mask);
    }

    public static bool isKeyCurrentUp(KeyCode key, FOCUS_MASK mask = FOCUS_MASK.SCENE)
    {
        return mInputSystem.isKeyCurrentUp(key, mask);
    }

    public static bool isKeyDown(KeyCode key, FOCUS_MASK mask = FOCUS_MASK.SCENE)
    {
        return mInputSystem.isKeyDown(key, mask);
    }

    public static bool isKeyUp(KeyCode key, FOCUS_MASK mask = FOCUS_MASK.SCENE)
    {
        return mInputSystem.isKeyUp(key, mask);
    }
    ...
    public static bool atProcedure(Type type)
    {
        return getCurScene().atProcedure(type);
    }

    public static bool atProcedure<T>() where T : SceneProcedure
    {
        return getCurScene().atProcedure(typeof(T));
    }

    // 百分比一般用于属性增幅之类的
    public static string toPercent(string value, int precision = 1)
    {
        return FToS(SToF(value) * 100, precision);
    }

    public static string toPercent(float value, int precision = 1)
    {
        return FToS(value * 100, precision);
    }

    // 几率类的一般是万分比的格式填写的
    public static string toProbability(string value)
    {
        return FToS(SToF(value) * 0.01f);
    }

    public static string toProbability(float value)
    {
        return FToS(value * 0.01f);
    }
    ...
    public static myUGUIObject getUGUIRoot()
    {
        return mLayoutManager?.getUIRoot();
    }

    public static Canvas getUGUIRootComponent()
    {
        return mLayoutManager?.getUIRoot()?.getCanvas();
    }

    public static Camera getUICamera()
    {
        return mCameraManager.getUICamera()?.getCamera();
    }
    ...
    public static void LIST<T>(out List<T> list, IEnumerable<T> initList = null)
    {
        ...
        if (GameEntry.getInstance().frameworkParam.enablePoolStackTrace)
        {
