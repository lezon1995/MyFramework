...
{
    int sidhgsg; // 密文,实际使用的随机一个密文存储
    int isdhy34; // 密文,实际使用的随机一个密文存储
    int hibsd; // 密文,实际使用的随机一个密文存储
    int gikowjeg; // 明文,用于校验内存是否被修改过
    int sgkn; // 密文,实际使用的随机一个密文存储
    int jgo2; // 密文,实际使用的随机一个密文存储
    int hgwikg; // 密文,实际使用的随机一个密文存储
    int wghwe; // 当前使用的密文下标
    int gnwe; // 密文,实际使用的随机一个密文存储
    int woghjwe; // 密钥0
    int gwijhge; // 密文,实际使用的随机一个密文存储
    int gikoweg; // 密钥1
    static int mValue0 = 135386587; // gikoweg的最小范围
    static int mValue1 = 335456345; // gikoweg的最大范围
    static int mValue2 = 212346587; // woghjwe的最小范围
    static int mValue3 = 311156345; // woghjwe的最大范围
    static int mValue5 = 437; // woghjwe与gikoweg的和需要整除的值,用于校验
