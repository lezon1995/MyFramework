...
{
    int erhgu; // 密文,实际使用的随机一个密文存储
    int wgjb; // 密文,实际使用的随机一个密文存储
    int gjkwqbg; // 密文,实际使用的随机一个密文存储
    int khsgh; // 密文当前的下标
    int qgoqjg; // 密钥1
    int gjkqwbeg; // 密文,实际使用的随机一个密文存储
    int geqbw; // 密文,实际使用的随机一个密文存储
    int gjwkehgwe; // 密文,实际使用的随机一个密文存储
    int qwfb; // 密钥0
    int huihg; // 密文,实际使用的随机一个密文存储
    int asgihfasg; // 明文,用于校验内存是否被修改过
    int gjqweg; // 密文,实际使用的随机一个密文存储
    static int mValue0 = 56859348; // qwfb的最小范围
    static int mValue1 = 91239999; // qwfb的最大范围
    static int mValue3 = 33776454; // qgoqjg的最小范围
    static int mValue4 = 81239999; // qgoqjg的最大范围
    static int mValue5 = 737; // qgoqjg与qwfb的和需要整除的值,用于校验
