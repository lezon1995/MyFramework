...
namespace MoreMountains
{
    ...
    ///     然后调用 OverlayMenuService.Register(panel, binder)。
    ///   • 任何想"按 APlayer 上下文打开商店面板"的地方都调
    ///     OverlayMenuService.Instance.Open(player)。
    ///   • 关闭时调 Close(player)。
    /// </summary>
    public sealed class OverlayMenuService
    {
        static OverlayMenuService sInstance;

        public static OverlayMenuService Instance => sInstance ??= new();

        OverlayMenuBinder _binder;
        APlayer _boundPlayer;
        ARoom _room;
        bool _opened;

        public OverlayMenuBinder Binder => _binder;
        public APlayer CurrentPlayer => _boundPlayer;
        public bool Opened => _opened;

        public void Register(OverlayMenuBinder binder)
        {
            _binder = binder;
        }
        ...
        public void Open(ARoom room, APlayer player)
        {
            if (_binder == null)
            {
                logError("OverlayMenuService.Open: panel / binder not registered.");
                return;
            }
        ...
        public void Open()
        {
            ...
            logError("OverlayMenuService.Open: no player bound; please call Open(APlayer) or Register then Bind.");
        }
