...
// State on the side where the timer is decreased on the screen
    ...
    /// </summary>
    public class Toast : MonoBehaviour, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        // Be careful when changing them at runtime, as static variables, this will override existing settings
        public static bool isStopped; // Set this variable to stop the timer of messages on screen 
        ...
        public static bool isHiding; // Check if you can animate with Fade effect a message
        public static bool isCanvasGroup; // Check if it has a CanvasGroup in toastNotification object

        static Transform messagePrefab; // Get the public prefab in a static variable
        static Transform toastNotification; // Get the object that this script is linked to this static variable
        ...
        [Tooltip("Minimum time that all messages will be displayed.")]
        public float _minimumMessageTime = 3;

        [Tooltip("Margin X and Y on the corners. Margin X doesn't works with centralized messages.")]
        ...
        // ----------- OVERLOADS ----------- 
        // Message text is the minimum necessary
        public static void Show(string msg)
        {
            Show(msg, minimumMessageTime, "");
        }

        // Text and timer
        public static void Show(string msg, float time)
        {
            Show(msg, time, "");
        }

        // Text and icon
        public static void Show(string msg, string iconName)
        {
            Show(msg, minimumMessageTime, iconName);
        }
        // ---------------------------------- 

        public static void Show(string msg, float time = -1, string iconName = "")
        {
            Debug.Log(msg);
            ...
            if (isCanvasGroup) toastNotification.GetComponent<CanvasGroup>().alpha = 0; // Instantiate with zero alpha (invisible)

            // Get IMAGE component of text, icons, timer and background
            Text text = message.Find("Text").GetComponent<Text>();
            ...
            SetMessageColor();
            SetupInvokeMessage();
            ResetToastNotificationPosition();
            SetMessagePositionOnScreen();

            // Set the message text and his alignment
            // You can change the text alignment at any time using something like:
            // GameObject.Find("ToastNotification").transform.Find("Text").GetComponent<TextMeshProUGUI>().alignment = ...
            void SetText()
            {
                text.text = msg;
            ...
            // or adapt the message so as not to display it if  is the parameter was not passed
            void SetMessageIcon()
            {
                ...
                if (iconName != "")
                {
                    ...
                    selectedIcon = icons.Find(iconName).transform.GetComponent<Image>(); // Find icon in child
            ...
            {
                ToastMessage toastMessage = message.GetComponent<ToastMessage>();
                toastMessage.timerRectTransform = timer.GetComponent<RectTransform>();
                toastMessage.messageTime = time;

                timer.enabled = showTimerRender;
                timer.enabled = time != 0 && timer.enabled;
                toastMessage.leftToRight = timerDirection == TimerDirection.LeftToRight;
            }

            // Guides the position of the objects, as it may change in size, for example, if an icon was not passed
            void ResetToastNotificationPosition()
            {
