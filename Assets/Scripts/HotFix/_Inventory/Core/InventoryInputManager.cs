...
namespace MoreMountains.InventoryEngine
{
    ...
    /// </summary>
    public class InventoryInputManager : MonoBehaviour, IEvent<InventoryEvent>
    {
        [Header("Settings")]
        ...
        [Header("Targets")]
        [MMInformation("Bind here your inventory container (the CanvasGroup that you want to turn on/off when opening/closing the inventory), your main InventoryDisplay, and the overlay that will be displayed under the InventoryDisplay when opened.")]
        ...
        public float OverlayInactiveOpacity;

        [Header("Start Behaviour")]
        [MMInformation("If you set HideContainerOnStart to true, the TargetInventoryContainer defined right above this field will be automatically hidden on Start, even if you've left it visible in Scene view. Useful for setup.")]
...
#else
        [Header("Key Mapping")]
        [MMInformation("Here you need to set the various key bindings you prefer. There are some by default but feel free to change them.")]
        /// the key used to open/close the inventory
        ...
        protected bool _hotbarInputPressed;
        ...
        protected virtual void Start()
        {
            ...
            foreach (var go in FindObjectsOfType<InventoryHotbar>())
            {
            ...
            if (HideContainerOnStart)
            {
                ...
                if (Overlay)
                {
                    Overlay.alpha = OverlayInactiveOpacity;
                }
        ...
        public void InputActivate() => InputDetectionActive = true;
        public void InputDeactivate() => InputDetectionActive = false;
        ...
        {
            _currentSelection = UnityEngine.EventSystems.EventSystem.current.currentSelectedGameObject;
            if (_currentSelection == null)
                return;

            _currentInventorySlot = _currentSelection.GetComponent<InventorySlot>();
        ...
        protected virtual void HandleButtons()
        {
            ...
            if (CurrentlySelectedInventorySlot)
            {
                ...
                if (_isEquipUseButtonNotNull)
                    SetButtonState(EquipUseButton, (CurrentlySelectedInventorySlot.Usable() || CurrentlySelectedInventorySlot.Equippable()) && CurrentlySelectedInventorySlot.EquipUseButtonShouldShow());
                ...
                if (_isMoveButtonNotNull)
                    SetButtonState(MoveButton, CurrentlySelectedInventorySlot.MoveButtonShouldShow());
        ...
        public virtual void OpenInventory()
        {
            if (CloseList.Count > 0)
            {
                ...
                {
                    InventoryEvent.Trigger(Inventory.Events.InventoryCloseRequest, null, "", null, 0, 0, playerID);
                }
            }
            ...
            InventoryEvent.Trigger(Inventory.Events.InventoryOpens, null, TargetInventoryDisplay.TargetInventoryName, TargetInventoryDisplay.Inventory[0], 0, 0, TargetInventoryDisplay.PlayerID);
            MMGameEvent.Trigger("inventoryOpens");
            InventoryIsOpen = true;

            Timing.RunCoroutine(MMFade.FadeCanvasGroup(TargetInventoryContainer, 0.2f, 1f));
            Timing.RunCoroutine(MMFade.FadeCanvasGroup(Overlay, 0.2f, OverlayActiveOpacity));
        }
        ...
        public virtual void CloseInventory()
        {
            ...
            InventoryEvent.Trigger(Inventory.Events.InventoryCloses, null, TargetInventoryDisplay.TargetInventoryName, null, 0, 0, TargetInventoryDisplay.PlayerID);
            MMGameEvent.Trigger("inventoryCloses");
            InventoryIsOpen = false;

            Timing.RunCoroutine(MMFade.FadeCanvasGroup(TargetInventoryContainer, 0.2f, 0f));
            Timing.RunCoroutine(MMFade.FadeCanvasGroup(Overlay, 0.2f, OverlayInactiveOpacity));
        }
        ...
        protected virtual void HandleInventoryInput()
        {
            ...
            {
                if (_currentInventoryDisplay.GoToInventory(-1))
                {
            ...
            if (_moveKeyPressed)
            {
                if (CurrentlySelectedInventorySlot)
                {
                    if (CurrentlySelectedInventorySlot.Item && CurrentlySelectedInventorySlot.Item.DisplayProperties.AllowMoveShortcut)
                    {
            ...
            {
                if (CurrentlySelectedInventorySlot.Item && CurrentlySelectedInventorySlot.Item.DisplayProperties.AllowEquipUseShortcut)
                {
                    EquipOrUse();
                }
            }
            ...
            if (_equipKeyPressed)
            {
                if (CurrentlySelectedInventorySlot)
                {
                    if (CurrentlySelectedInventorySlot.Item && CurrentlySelectedInventorySlot.Item.DisplayProperties.AllowEquipShortcut)
                    {
            ...
            if (_useKeyPressed)
            {
                if (CurrentlySelectedInventorySlot)
                {
                    if (CurrentlySelectedInventorySlot.Item && CurrentlySelectedInventorySlot.Item.DisplayProperties.AllowUseShortcut)
                    {
            ...
            if (_dropKeyPressed)
            {
                if (CurrentlySelectedInventorySlot)
                {
                    if (CurrentlySelectedInventorySlot.Item && CurrentlySelectedInventorySlot.Item.DisplayProperties.AllowDropShortcut)
                    {
        ...
        {
            if (e.PlayerID != TargetInventoryDisplay.PlayerID)
                return;

            if (e.Events == Inventory.Events.InventoryCloseRequest)
            {
                CloseInventory();
            }
        }
