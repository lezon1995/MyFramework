...
namespace MoreMountains.TopDownEngine
{
        ...
        // , IEvent<TopDownEngineEvent>
    {
        ...
        [MMEnumCondition(nameof(ReticleType), (int)ReticleTypes.Scene, (int)ReticleTypes.UI)]
        [Tooltip("the gameobject to display as the aim's reticle/crosshair. Leave it blank if you don't want a reticle")]
        public GameObject Reticle;

        [MMEnumCondition(nameof(ReticleType), (int)ReticleTypes.Scene)]
        ...
        [MMInspectorGroup("Camera Target")]
        ...
        public virtual Quaternion CurrentRotation => transform.rotation;

        /// the weapon's current direction
        public virtual Vector3 CurrentAim => _currentAim;

        /// the weapon's current direction, absolute (flip independent)
        public virtual Vector3 CurrentAimAbsolute => _currentAimAbsolute;
        ...
        protected virtual void Initialization()
        {
            ...
            switch (RotationMode)
            {
                case RotationModes.Strict4Directions:
                    _possibleAngleValues = new float[5] { -180F, -90F, 0F, 90F, 180F };
                    break;
                case RotationModes.Strict8Directions:
                    _possibleAngleValues = new float[9] { -180F, -135F, -90F, -45F, 0F, 45F, 90F, 135F, 180F };
                    break;
            }

            _initialRotation = transform.rotation;
            InitializeReticle();
            _playerPlane = new(Vector3.up, Vector3.zero);
            _initialized = true;
        }
        ...
        {
            var inputManager = _weapon.Owner.Input;
            if (inputManager)
            {
                var forceWeaponMode = inputManager.ForceWeaponMode;
                if (forceWeaponMode && AimControl is not AimControls.Off)
                {
                    AimControl = inputManager.WeaponForcedMode;
                }
                else if (!forceWeaponMode && inputManager.IsMobile && AimControl is AimControls.Mouse)
                {
