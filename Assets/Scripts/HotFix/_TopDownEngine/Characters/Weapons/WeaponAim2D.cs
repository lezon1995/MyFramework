...
namespace MoreMountains
{
    ...
    [AddComponentMenu("TopDown Engine/Weapons/WeaponAim2D")]
    public class WeaponAim2D : WeaponAim
    {
        ...
        protected override void Initialization()
        {
            ...
            if (_weapon.Owner.FindAbility<CharacterOrientation2D>(out var orientation))
            {
                ...
                {
                    Character.FacingDirections.East => Vector2.right,
                    Character.FacingDirections.North => Vector2.up,
                    Character.FacingDirections.West => Vector2.left,
                    Character.FacingDirections.South => Vector2.down,
                    _ => _lastNonNullMovement
                };
            }
        }
        ...
        protected override void GetCurrentAim()
        {
            ...
            if (_weapon.Owner.Input == null && _weapon.Owner.CharacterType == Character.Types.Player)
                return;
            ...
            switch (AimControl)
            {
                ...
                case AimControls.PrimaryMovement:
                    if (_weapon.Owner == null || _weapon.Owner.Input == null)
                        return;

                    if (_weapon.Owner.Input.PrimaryMovement.magnitude > MinimumMagnitude)
                    {
                ...
                case AimControls.SecondaryMovement:
                    ...
                    if (_weapon.Owner.Input.SecondaryMovement.magnitude > MinimumMagnitude)
                    {
        ...
        {
            if (_weapon?.Owner?.Input == null)
                return;

            var input = _weapon.Owner.Input;
            if (_lastNonNullMovement == Vector2.zero)
            {
                _lastNonNullMovement = input.LastNonNullPrimaryMovement;
            }

            _inputMovement = input.PrimaryMovement;
        ...
        public virtual void GetSecondaryMovementAim()
        {
            ...
            if (_lastNonNullMovement == Vector2.zero)
            {
                _lastNonNullMovement = input.LastNonNullSecondaryMovement;
            }

            _inputMovement = input.SecondaryMovement;
            TestLastMovement();
        ...
        protected override void InitializeReticle()
        {
            ...
            switch (ReticleType)
            {
                ...
                case ReticleTypes.UI:
                {
                    // _reticle = Instantiate(Reticle);
                    // _reticle.transform.SetParent(GUIManager.Instance.MainCanvas.transform);
                    // _reticle.transform.localScale = Vector3.one;
                    // if (_reticle.TryGetComponent<MMUIFollowMouse>(out var followMouse))
                    // {
                    //     followMouse.TargetCanvas = GUIManager.Instance.MainCanvas;
                    // }
        ...
        protected override void MoveTarget()
        {
            ...
            if (MoveCameraTargetTowardsReticle)
            {
                ...
                {
                    _newCamTargetPosition = transform.position + _currentAimAbsolute.normalized * CameraTargetMaxDistance;
                    _newCamTargetDirection = _newCamTargetPosition - transform.position;
                    _newCamTargetPosition = transform.position + _newCamTargetDirection;
                    _newCamTargetPosition = Vector3.Lerp(_weapon.Owner.CameraTarget.transform.position, Vector3.Lerp(transform.position, _newCamTargetPosition, CameraTargetOffset), Time.deltaTime * CameraTargetSpeed);
