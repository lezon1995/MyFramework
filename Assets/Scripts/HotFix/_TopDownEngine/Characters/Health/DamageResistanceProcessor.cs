...
namespace MoreMountains
{
    ...
    public class DamageResistanceProcessor : TopDownMonoBehaviour
    {
        ...
        public bool IgnoreUnknownDamageTypes;
        ...
        protected virtual void Initialization()
        {
            ...
            {
                var damageResistances = GetComponentsInChildren<DamageResistance>(includeInactive: !IgnoreDisabledResistances);
                if (damageResistances.Length > 0)
                {
                    DamageResistanceList = damageResistances.ToList();
                }
            }
        ...
        public virtual void InterruptDamageOverTime(DamageType damageType)
        {
            foreach (DamageResistance resistance in DamageResistanceList)
            {
                if (resistance.gameObject.activeInHierarchy && (resistance.DamageTypeMode == DamageType.Modes.Base || resistance.TypeResistance == damageType) && resistance.InterruptibleFeedback)
                {
                    resistance.OnDamageReceived.Stop();
                }
            }
        }
        ...
        public virtual bool CheckPreventCharacterConditionChange(DamageType typedDamage)
        {
            foreach (DamageResistance resistance in DamageResistanceList)
            {
                ...
                if (typedDamage == null)
                {
                    if (resistance.DamageTypeMode == DamageType.Modes.Base && resistance.PreventCharacterConditionChange)
                        return true;
                }
                else
                {
                    if (resistance.TypeResistance == typedDamage && resistance.PreventCharacterConditionChange)
                        return true;
                }
            }
        ...
        public virtual bool CheckPreventMovementModifier(DamageType typedDamage)
        {
            foreach (DamageResistance resistance in DamageResistanceList)
            {
                ...
                if (typedDamage == null)
                {
                    if (resistance.DamageTypeMode == DamageType.Modes.Base && resistance.PreventMovementModifier)
                        return true;
                }
                else
                {
                    if (resistance.TypeResistance == typedDamage && resistance.PreventMovementModifier)
                        return true;
                }
            }
        ...
        public virtual bool CheckPreventKnockback(List<TypedDamage> typedDamages)
        {
            if ((typedDamages == null) || (typedDamages.Count == 0))
            {
                foreach (DamageResistance resistance in DamageResistanceList)
                {
                    ...
                    if (resistance.DamageTypeMode == DamageType.Modes.Base && resistance.ImmuneToKnockback)
                        return true;
                }
            }
            else
            {
                foreach (TypedDamage typedDamage in typedDamages)
                {
                    foreach (DamageResistance resistance in DamageResistanceList)
                    {
                        ...
                        {
                            if (resistance.TypeResistance == typedDamage.AssociatedDamageType && resistance.ImmuneToKnockback)
                                return true;
                        }
                    }
                }
