...
namespace MoreMountains
{
    ...
    public class CharacterDash2D : CharacterAbility
    {
        ...
        protected virtual void HandleDashMode()
        {
            switch (DashMode)
            {
                ...
                case DashModes.SecondaryMovement:
                    _dashDestination = transform.position + (Vector3)_inputManager.SecondaryMovement.normalized * DashDistance;
                    break;

                case DashModes.MousePosition:
                    var position = _mainCamera.ScreenToWorldPoint(_inputManager.MousePosition);
                    position.z = transform.position.z;
                    _dashDestination = transform.position + (position - transform.position).normalized * DashDistance;
                    break;
            }
        }
        ...
        public override void OnUpdate(float dt1)
        {
