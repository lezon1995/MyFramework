namespace MoreMountains
{
    ...
    public enum BallType
    {
        ...
        //裂变
        Fission = 11000, //每次撞击会分裂出2个小球，持续3秒
        FissionMini = 11001,

        //复制
        Duplicate = 11100, //每次撞击有几率复制出一个球，持续3秒

        //飞弹
        Missile = 11200, //每次撞击会从玩家身后发射一道轨迹子弹在0.75秒后命中敌人

        //手里剑
        Shuriken = 11300, //每次命中敌人时会往附近的敌人弹射

        //流血
        Bleed = 11400, //命中施加2层流血。每层流血将使敌人在受到弹珠攻击时额外承受1+0.1AP点伤害（最高8层）

        //电锯
        ChainSaw = 11500, //穿透敌人，穿透过程中持续造成伤害

        //金属
        Iron = 11600, //钢铁，100%暴击率，移速慢

        //幽灵，
        Ghost = 11700, //伤害低，移速快，攻速快

        //阴阳
        YinYang = 11800, //命中时偷取目标1点护甲和1点魔抗，附加自身50%护甲的物理伤害和自身50%魔抗的魔法伤害

        //黑洞
        BlackHole = 11900, //撞击生成一个黑洞，黑洞范围内敌人不断被拉向中心

        //炸弹
        Bomb = 12000, //每4次撞击触发1次圆形爆炸

        //金币
        Coin = 12100, //命中敌人有几率掉落金币，金币越多伤害越高

        //磁铁
        Magnet = 12200, //经过经验和金币附近会自动拾取

        //传送
        Teleport = 12300, //撞击后会随机传送到一个位置

        //创造
        Creation = 12400, //命中过的砖块在3秒内被摧毁时，会在原地创建一个障碍物，存活5秒

        //行星
        Planet = 12500, //飞行方式是环绕自身

        //影子球
        Shadow = 12600, //进入砖块内部反弹，砖块阵亡后才会弹出

        //吸血鬼
        Vampire = 12700, //将造成伤害的15%转为血量储存起来，最多储存5+0.2AP点，玩家碰到时恢复已存储的生命值。

        //护盾
        Shield = 12800, //命中时获得1+0.2AP点护盾

        //能量射线
        EnergyRay = 12900, //每隔4秒释放一道持续2秒的旋转能量射线

        //足球
        Football = 13000, //冲刺时碰到足球会大幅提高足球的速度，速度+200%，速度越快伤害越高。
        ...
        IceFrost_Electricity, //IceFrost + ElectricityStrike 霜电打击，对场上随机1名敌人释放霜电打击，在霜电路径上生成一个持续3秒的霜冻区域，对区域内的敌人每0.5秒受到伤害并施加1层霜冻，3层时冻住
