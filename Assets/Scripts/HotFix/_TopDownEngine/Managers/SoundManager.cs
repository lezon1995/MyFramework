...
namespace MoreMountains.TopDownEngine
{
    ...
    /// </summary>
    [Obsolete("This SoundManager is now obsolete, and has been replaced by the bigger, better, faster MMSoundManager. It will be removed definitely in an upcoming update. You should remove this one from this scene, and add a MMSoundManager in its place.")]
        ...
        , IEvent<MMGameEvent>
    {
        ...
        /// <returns></returns>
        public virtual AudioSource PlaySound(AudioClip sfx, Vector3 location, float pitch, float pan, float spatialBlend = 0.0f, float volumeMultiplier = 1.0f, bool loop = false, AudioSource reuseSource = null, AudioMixerGroup audioGroup = null, int priority = 128)
        {
            ...
            if (!loop && !reuseSource)
            {
                // we destroy the host after the clip has played (if it not tag for reuse).
                Destroy(temporaryAudioHost, sfx.length);
            }
        ...
        {
            var settings = MMSaveLoadManager.Load<SoundSettings>(_saveFileName, _saveFolderName);
            if (settings != null)
            {
                Settings = settings;
            }
        ...
        {
            PlaySound(clipToPlay, transform.position, pitch, 0.0f, 0.0f, volume, false, audioGroup: audioGroup, priority: priority);
        }
