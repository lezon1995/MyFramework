...
namespace MoreMountains.Feedbacks
{
	...
	/// </summary>
	public abstract class MMSpringVector2Component<T> : MMSpringComponentBase, IEvent<MMSpringVector2Event> where T:Component
	{
		[MMInspectorGroup("Target")] 
		public T Target;
		
		[MMInspectorGroup("Channel & TimeScale")] 
		...
		[MMInspectorGroup("Spring Settings")]
		[Header("SpringVector2")]
		public MMSpringVector2 SpringVector2;
		
		[MMInspectorGroup("Randomness")]
		...
		public void onEvent(MMSpringVector2Event e)
		{
			bool eventMatch = e.ChannelData != null && MMChannel.Match(e.ChannelData, ChannelMode, Channel, MMChannelDefinition);
			bool targetMatch = e.TargetSpring != null && e.TargetSpring.Equals(this);
			if (!eventMatch && !targetMatch)
			{
				return;
			}
			
			if (e.OverrideDamping)
			{
				SpringVector2.SetDamping(e.NewDamping);
			}
			if (e.OverrideFrequency)
			{
				SpringVector2.SetFrequency(e.NewFrequency);
			}
			switch (e.Command)
			{
				case SpringCommands.MoveTo:
					MoveTo(e.MoveToValue);
					break;
				case SpringCommands.MoveToAdditive:
					MoveToAdditive(e.MoveToValue);
					break;
				case SpringCommands.MoveToSubtractive:
					MoveToSubtractive(e.MoveToValue);
					break;
				case SpringCommands.MoveToRandom:
					MoveToRandom(e.MoveToRandomValueMin, e.MoveToRandomValueMax);
					break;
				case SpringCommands.MoveToInstant:
					MoveToInstant(e.MoveToValue);
					break;
				case SpringCommands.Bump:
					Bump(e.BumpAmount);
					break;
				case SpringCommands.BumpRandom:
					BumpRandom(e.BumpAmountRandomValueMin, e.BumpAmountRandomValueMax);
					break;
		...
		protected override void Awake()
		{
			...
			base.Awake();
			this.addListener<MMSpringVector2Event>();
		}

		protected void OnDestroy()
		{
			this.removeListener<MMSpringVector2Event>();
		}
