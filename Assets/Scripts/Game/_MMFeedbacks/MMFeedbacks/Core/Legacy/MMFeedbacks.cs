...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMFeedbacks : MonoBehaviour
    {
        ...
        public virtual async Task PlayFeedbacksTask(Vector3 position, float feedbacksIntensity = 1.0f, bool forceRevert = false)
        {
        ...
        /// <returns></returns>
        public virtual IEnumerator<float> PlayFeedbacksCoroutine(Vector3 position, float feedbacksIntensity = 1.0f, bool forceRevert = false)
        {
            PlayFeedbacks(position, feedbacksIntensity, forceRevert);
            while (IsPlaying)
            {
        ...
        protected virtual IEnumerator<float> HandleInitialDelayCo(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
        {
            IsPlaying = true;
            yield return Timing.WaitForSeconds(ComputedInitialDelay);
            PreparePlay(position, feedbacksIntensity, forceRevert);
        }
        ...
        /// <param name="feedbacksIntensity"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
        {
            yield return Timing.WaitForOneFrame;
        }
        ...
        protected bool FeedbackCanPlay(MMFeedback feedback)
        {
            ...
            else if (((Direction == Directions.TopToBottom) && (feedback.Time.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenForwards))
