...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMVectorLabelsAttributeDrawer : PropertyDrawer
    {
        ...
        protected T[] DrawFields<T>(Rect rect, T[] vector, string mainLabel, Func<Rect, GUIContent, T, T> fieldDrawer, MMFVectorAttribute vectors)
        {
            T[] result = vector;
        ...
        public static object MMFGetObjectValue(this SerializedProperty property)
        {
            ...
            foreach (var element in elements)
            {
                ...
                else
                {
                    ...
                    int elementIndex = Convert.ToInt32(element.Substring(element.IndexOf("[")).Replace("[", "").Replace("]", ""));
