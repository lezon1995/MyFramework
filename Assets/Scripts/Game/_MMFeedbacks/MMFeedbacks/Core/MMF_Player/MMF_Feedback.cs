...
namespace MoreMountains.Feedbacks
{
    ...
    public abstract class MMF_Feedback
    {
        ...
        [Tooltip("whether to broadcast this feedback's message using an int or a scriptable object. Ints are simple to setup but can get messy and make it harder to remember what int corresponds to what. " +
        ...
        [Tooltip("the MMChannel definition asset to use to broadcast this feedback. The shaker will have to reference that same MMChannel definition to receive events - to create a MMChannel, " +
        ...
        public virtual IEnumerator<float> Pause => null;
        ...
        public virtual bool InCooldown => (Time.CooldownDuration > 0f) &&
                                          (FeedbackTime - _lastPlayTimestamp < Time.CooldownDuration);
        ...
        {
            float result = Time.ConstantIntensity ? 1f : intensity;
        ...
        public virtual bool InScaledTimescaleMode
        {
            get
            {
                ...
                return (Time.TimescaleMode == TimescaleModes.Scaled);
            }
        }
        ...
        public virtual float FeedbackTime
        {
            get
            {
                ...
                if (Time.UseScriptDrivenTimescale)
                {
                    return Time.ScriptDrivenTime;
                }
        ...
        public virtual float FeedbackDeltaTime
        {
            get
            {
                if (Time.UseScriptDrivenTimescale)
                {
                    return Time.ScriptDrivenDeltaTime;
                }
        ...
        public virtual bool FeedbackPlaying => FeedbackStartedAt > 0f && UnityEngine.Time.time - FeedbackStartedAt < FeedbackDuration;
        ...
        {
            yield return Timing.WaitUntilDone(WaitFor(ApplyTimeMultiplier(Time.InitialDelay)));
            RegularPlay(position, feedbacksIntensity);
        }
        ...
        protected virtual void RegularPlay(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            if (Time.LimitPlayCount && (Time.PlayCount >= Time.MaxPlayCount))
                return;
        ...
        protected virtual IEnumerator<float> InfinitePlay(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            while (true)
            {
                yield return Timing.WaitUntilDone(TriggerRepeatedPlay(position, feedbacksIntensity));
            }
        }
        ...
        protected virtual IEnumerator<float> TriggerRepeatedPlay(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            if (Time.Sequence == null)
            {
                TriggerCustomPlay(position, feedbacksIntensity);
                float repeatStartTime = UnityEngine.Time.time;

                float repeatDuration = Time.DelayBetweenRepeats + FeedbackDuration;
                if (_repeatOffset <= Time.DelayBetweenRepeats)
                {
                    repeatDuration = Time.DelayBetweenRepeats + FeedbackDuration - _repeatOffset;
                }

                yield return Timing.WaitUntilDone(WaitFor(repeatDuration));
                yield return Timing.WaitForOneFrame;
                _repeatOffset = (UnityEngine.Time.time - repeatStartTime - repeatDuration);
            }
            else
            {
                _sequenceCoroutine = Timing.RunCoroutine(SequenceCoroutine(position, feedbacksIntensity));
                float delay = ApplyTimeMultiplier(Time.DelayBetweenRepeats) + Time.Sequence.Length;
                yield return Timing.WaitUntilDone(WaitFor(delay));
            }
        }
        ...
        protected virtual IEnumerator<float> SequenceCoroutine(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            else
            {
                while (FeedbackTime - timeStartedAt < Time.Sequence.Length)
                {
                    ...
                    {
                        if ((item.ID == Time.TrackID) && (item.Timestamp >= lastFrame) &&
        ...
        public virtual void ForceInitialValue(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            ...
            {
                Timing.RunCoroutine(ForceInitialValueDelayedCo(position, feedbacksIntensity));
            }
            else
            {
        ...
        /// <param name="feedbacksIntensity"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> ForceInitialValueDelayedCo(Vector3 position, float feedbacksIntensity = 1.0f)
        {
            Play(position, feedbacksIntensity);
            yield return Timing.WaitForOneFrame;
            Stop(position, feedbacksIntensity);
        ...
        {
            _playsLeft = Time.NumberOfRepeats + 1;
            if (Time.SetPlayCountToZeroOnReset)
            {
                ResetPlayCount();
            }
        ...
        /// <param name="delay"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> WaitFor(float delay)
        {
        ...
        {
            if (Time is { ContributeToTotalDuration: false })
            {
                _totalDuration = 0f;
                return;
            }
            ...
            if (Time.NumberOfRepeats != 0)
            {
                ...
                totalTime += (Time.NumberOfRepeats * delayBetweenRepeats);
            }
        ...
        public virtual bool NormalPlayDirection
        {
            get
            {
                return Time.PlayDirection switch
                {
