...
namespace MoreMountains.Feedbacks
{
    ...
    public class MMF_Player : MMFeedbacks
    {
        ...
        protected override void OnEnable()
        {
            ...
            if (AutoPlayOnEnable && Application.isPlaying)
            {
                ...
                if (Time.frameCount < 2)
                {
                    ...
                    Timing.RunCoroutine(PlayFeedbacksAfterFrames(2));
                }
                else
                {
                    PlayFeedbacks();
        ...
        /// <returns></returns>
        public virtual IEnumerator<float> PlayFeedbacksAfterFrames(int framesAmount)
        {
            yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitForFrames(framesAmount));
            PlayFeedbacks();
        }
        ...
        /// <returns></returns>
        public override IEnumerator<float> PlayFeedbacksCoroutine(Vector3 position, float feedbacksIntensity = 1.0f, bool forceRevert = false)
        {
            PlayFeedbacks(position, feedbacksIntensity, forceRevert);
            while (IsPlaying)
            {
        ...
        protected override void PlayFeedbacksInternal(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
        {
            ...
            {
                this.enabled = false;
                Timing.RunCoroutine(FrameOnePlayCo(position, feedbacksIntensity, forceRevert));
                return;
            }

            if (InitialDelay > 0f)
            {
                Timing.RunCoroutine(HandleInitialDelayCo(position, feedbacksIntensity, forceRevert));
            }
            else
            {
        ...
        protected virtual IEnumerator<float> FrameOnePlayCo(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
        {
            yield return Timing.WaitForOneFrame;
            this.enabled = true;
            _startTime = GetTime();
            _lastStartAt = _startTime;
            IsPlaying = true;
            yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitForUnscaled(ComputedInitialDelay), Segment.RealtimeUpdate);
            PreparePlay(position, feedbacksIntensity, forceRevert);
        }
        ...
        protected override void PreparePlay(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
        {
            ...
            else
            {
                ...
                Timing.RunCoroutine(PausedFeedbacksCo(position, feedbacksIntensity));
            }
        }
        ...
        protected override IEnumerator<float> HandleInitialDelayCo(Vector3 position, float feedbacksIntensity, bool forceRevert = false)
        {
            IsPlaying = true;

            if (PlayerTimescaleMode == TimescaleModes.Scaled)
            {
                yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitFor(ComputedInitialDelay), Segment.RealtimeUpdate);
            }
            else
            {
                yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitForUnscaled(ComputedInitialDelay), Segment.RealtimeUpdate);
        ...
        /// <param name="feedbacksIntensity"></param>
        /// <returns></returns>
        protected override IEnumerator<float> PausedFeedbacksCo(Vector3 position, float feedbacksIntensity)
        {
            IsPlaying = true;
            ...
            while (i >= 0 && i < count)
            {
                ...
                if ((feedback.Active && feedback.ScriptDrivenPause) || InScriptDrivenPause)
                {
                    InScriptDrivenPause = true;

                    bool inAutoResume = feedback.ScriptDrivenPauseAutoResume > 0f;
                    float scriptDrivenPauseStartedAt = GetTime();
                    float autoResumeDuration = feedback.ScriptDrivenPauseAutoResume;
                ...
                if (feedback.Active
                    && (feedback.HoldingPause || feedback.LooperPause)
                    && feedback.ShouldPlayInThisSequenceDirection)
                {
                ...
                // plays the feedback
                if (FeedbackCanPlay(feedback))
                {
                    feedback.Play(position, feedbacksIntensity);
                }

                // Handles pause
                if (feedback.Pause != null && feedback.Active && feedback.ShouldPlayInThisSequenceDirection && !SkippingToTheEnd)
                {
                    bool shouldPause = true;
                    if (feedback.Chance < 100)
                    {
                        float random = Random.Range(0f, 100f);
                        if (random > feedback.Chance)
                        {
                            shouldPause = false;
                        }
                    }
                    ...
                    if (shouldPause)
                    {
                        yield return Timing.WaitUntilDone(feedback.Pause);
                ...
                if (feedback.Active)
                {
                    if (feedback.Pause == null && feedback.ShouldPlayInThisSequenceDirection && !feedback.Time.ExcludeFromHoldingPauses)
                    {
                        float feedbackDuration = feedback.TotalDuration;
                        _holdingMax = Mathf.Max(feedbackDuration, _holdingMax);
                    }
                }

                // handles looper
                var looper = (MMF_Looper)feedback;
                if (feedback.LooperPause
                    && feedback.Active
                    && feedback.ShouldPlayInThisSequenceDirection
                    && (looper.NumberOfLoopsLeft > 0 || looper.InInfiniteLoop))
                {
        ...
        protected virtual IEnumerator<float> SkipToTheEndCo()
        {
        ...
        public virtual void ForceInitialValues()
        {
            ...
            for (int i = count - 1; i >= 0; i--)
            {
                ...
                if (feedback is { Active: true })
                {
                    feedback.ForceInitialValue(transform.position, FeedbacksIntensity);
                }
            }
        }
        ...
        {
            Timing.RunCoroutine(SkipToTheEndCo());
        }
        ...
        {
            InitializeFeedbackList();
            var feedback = (MMF_Feedback)Activator.CreateInstance(feedbackType);
            feedback.Label = FeedbackPathAttribute.GetFeedbackDefaultName(feedbackType);
            feedback.Owner = this;
            feedback.Time = new MMFeedbackTiming();
            ...
            feedback.InitializeCustomAttributes();
            feedback.CacheRequiresSetup();
            return feedback;
        }
        ...
        private class MMF_FeedbackListCopy
        {
            ...
            public static List<MMF_Feedback> CopyFrom(MMF_Player source)
            {
                ...
                {
                    FeedbackList = source.FeedbacksList
                };
        ...
        public virtual bool HasAutomaticShakerSetup
        {
            get
            {
                ...
                {
                    var feedback = FeedbacksList[i];
                    if (feedback is { HasAutomaticShakerSetup: true })
                    {
                        return true;
                    }
                }
        ...
        public override bool HasFeedbackStillPlaying()
        {
            ...
            {
                var feedback = FeedbacksList[i];
                if ((feedback.IsPlaying && !feedback.Time.ExcludeFromHoldingPauses) || feedback.Time.RepeatForever || (feedback.Time.NumberOfRepeats > 0 && feedback.PlaysLeft > 0))
                {
                    return true;
                }
            }
        ...
        protected override void CheckForLoops()
        {
            ...
            {
                var feedback = FeedbacksList[i];
                if (feedback is { LooperPause: true, Active: true })
                {
                    ContainsLoop = true;
                    return;
                }
        ...
        protected virtual void ComputeNewRandomDurationMultipliers()
        {
            ...
            {
                var feedback = FeedbacksList[i];
                if (feedback is { RandomizeDuration: true })
                {
                    feedback.ComputeNewRandomDurationMultiplier();
                }
            }
        }
        ...
        {
            if (feedback.Time.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.Always)
                return true;

            if ((Direction == Directions.TopToBottom && feedback.Time.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenForwards)
                || (Direction == Directions.BottomToTop && feedback.Time.MMFeedbacksDirectionCondition == MMFeedbackTiming.MMFeedbacksDirectionConditions.OnlyWhenBackwards))
                return true;
        ...
        public virtual void ComputeCachedTotalDuration()
        {
            ...
            if (!_pauseFound)
            {
                foreach (MMF_Feedback feedback in FeedbacksList)
                {
                    ...
                    if (feedback is { Active: true, ShouldPlayInThisSequenceDirection: true })
                    {
            ...
            else
            {
                ...
                while (i >= 0 && i < FeedbacksList.Count && (iterations < maxIterationsSafety))
                {
                    ...
                    if (feedback is { Active: true, ShouldPlayInThisSequenceDirection: true })
                    {
                        ...
                        {
                            if (feedback.Time is { ContributeToTotalDuration: false })
                                continue;

                            // pause
                            var pause = (MMF_Pause)feedback;
                            if (feedback.HoldingPause)
                            {
                                intermediateTotal += ApplyTimeMultiplier(pause.PauseDuration);
                                total += intermediateTotal;
                                intermediateTotal = 0f;
                            }
                            else
                            {
                                currentPauseDelay += ApplyTimeMultiplier(pause.PauseDuration);
                            }

                            //loops
                            if (feedback.LooperStart)
                            {
                                lastLooperStart = i;
                            }
                            ...
                            if (feedback.LooperPause && feedback is MMF_Looper { NumberOfLoops: > 0 } looper)
                            {
                                ...
                                {
                                    lastLoopFoundAt = i;
                                    loopsLeft = looper.NumberOfLoops - 1;
                                }

                                if (looper.InfiniteLoop)
                                {
                                    _cachedTotalDuration = 999f;
                                    return;
                                }

                                if (looper.LoopAtLastPause)
                                {
                                    i = lastPauseFoundAt;
                                    total += intermediateTotal;
                                    intermediateTotal = 0f;
                        ...
                        {
                            float feedbackDuration = feedback.TotalDuration + currentPauseDelay;
