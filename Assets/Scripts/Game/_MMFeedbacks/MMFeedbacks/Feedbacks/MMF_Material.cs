...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Material : MMF_Feedback
	{
		...
		public virtual float GetTime() { return (ComputedTimescaleMode == TimescaleModes.Scaled) ? UnityEngine.Time.time : UnityEngine.Time.unscaledTime; }
		public virtual float GetDeltaTime() { return (ComputedTimescaleMode == TimescaleModes.Scaled) ? UnityEngine.Time.deltaTime : UnityEngine.Time.unscaledDeltaTime; }
        
		protected int _currentIndex;
		protected float _startedAt;
		protected CoroutineHandle[] _coroutines;
		...
		protected virtual void InitializeMaterials()
		{
			...
			_coroutines = new CoroutineHandle[RendererMaterialIndexes.Length];
		}
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (InterpolateTransition)
			{
				...
				{
					if (_coroutines[i] != default) { Timing.KillCoroutines(_coroutines[i]); }
					_coroutines[i] = Timing.RunCoroutine(TransitionMaterial(TargetRenderer.materials[RendererMaterialIndexes[i]], Materials[newIndex], RendererMaterialIndexes[i]));
				}
			}
			else
			{
		...
		/// <returns></returns>
		protected virtual IEnumerator<float> TransitionMaterial(Material originalMaterial, Material newMaterial, int materialIndex)
		{
			IsPlaying = true;
			_startedAt = GetTime();
		...
		protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1)
		{
			...
			if (Active && FeedbackTypeAuthorized && (_coroutines != null))
			{
				...
				for (int i = 0; i < RendererMaterialIndexes.Length; i++)
				{
					if (_coroutines[i] != default)
					{
						Timing.KillCoroutines(_coroutines[i]);    
					}
					_coroutines[i] = default;    
				}
			}
		}
