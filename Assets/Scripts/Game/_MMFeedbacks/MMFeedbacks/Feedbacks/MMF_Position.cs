...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Position : MMF_Feedback
	{
			...
			if (Active || Owner.AutoPlayOnEnable)
			{
				...
				switch (Mode)
				{
					case Modes.ToDestination:
						...
						if (_coroutine != default) { Timing.KillCoroutines(_coroutine); }
						_coroutine = Timing.RunCoroutine(MoveFromTo(AnimatePositionTarget, _initialPosition, _destinationPosition, FeedbackDuration, AnimatePositionTween));
						break;
					case Modes.AtoB:
						...
						if (_coroutine != default) { Timing.KillCoroutines(_coroutine); }
						_coroutine = Timing.RunCoroutine(MoveFromTo(AnimatePositionTarget, _workInitialPosition, _workDestinationPosition, FeedbackDuration, AnimatePositionTween));
						break;
					case Modes.AlongCurve:
						...
						if (_coroutine != default) { Timing.KillCoroutines(_coroutine); }
						_coroutine = Timing.RunCoroutine(MoveAlongCurve(AnimatePositionTarget, _workInitialPosition, FeedbackDuration, intensityMultiplier));
						break;
				}                    
			}
		}
		...
		protected virtual IEnumerator<float> MoveFromTo(GameObject movingObject, Vector3 pointA, Vector3 pointB, float duration, MMTweenType tweenType)
		{
			IsPlaying = true;
			float journey = NormalPlayDirection ? 0f : duration;
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
