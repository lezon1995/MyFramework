...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbackPosition : MMFeedback
	{
			...
			if (isActiveAndEnabled || _hostMMFeedbacks.AutoPlayOnEnable)
			{
				...
				switch (Mode)
				{
					case Modes.ToDestination:
						...
						_coroutine = Timing.RunCoroutine(MoveFromTo(AnimatePositionTarget, _initialPosition, _destinationPosition, FeedbackDuration, AnimatePositionCurve));
						break;
					case Modes.AtoB:
						if (!AllowAdditivePlays && (_coroutine != default))
						{
							return;
						}
						_coroutine = Timing.RunCoroutine(MoveFromTo(AnimatePositionTarget, InitialPosition, DestinationPosition, FeedbackDuration, AnimatePositionCurve));
						break;
					case Modes.AlongCurve:
						...
						float intensityMultiplier = Time.ConstantIntensity ? 1f : feedbacksIntensity;
						_coroutine = Timing.RunCoroutine(MoveAlongCurve(AnimatePositionTarget, InitialPosition, FeedbackDuration, intensityMultiplier));
						break;
				}                    
			}
		}
		...
		/// <returns></returns>
		protected virtual IEnumerator<float> MoveAlongCurve(GameObject movingObject, Vector3 initialPosition, float duration, float intensityMultiplier)
		{
			IsPlaying = true;
			...
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
			{
				...
				{
					journey += NormalPlayDirection ? UnityEngine.Time.unscaledDeltaTime : -UnityEngine.Time.unscaledDeltaTime;
				}
		...
		protected virtual IEnumerator<float> MoveFromTo(GameObject movingObject, Vector3 pointA, Vector3 pointB, float duration, AnimationCurve curve = null)
		{
			IsPlaying = true;
			float journey = NormalPlayDirection ? 0f : duration;
			while ((journey >= 0) && (journey <= duration) && (duration > 0))
		...
		{
			if (!Active || !FeedbackTypeAuthorized || (_coroutine == default))
			{
				return;
			}
			IsPlaying = false;
