...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbackPause : MMFeedback
	{
		...
		#endif
		public override IEnumerator<float> Pause { get => PauseWait(); }
		...
		/// <summary>
		/// An IEnumerator<float> used to wait for the duration of the pause, on scaled or unscaled time
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerator<float> PauseWait()
		{
			if (Time.TimescaleMode == TimescaleModes.Scaled)
			{
				yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitFor(PauseDuration));
			}
			else
			{
				yield return Timing.WaitUntilDone(MMFeedbacksCoroutine.WaitForUnscaled(PauseDuration), Segment.RealtimeUpdate);
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerator<float> PlayPause()
		{
			yield return Timing.WaitUntilDone(Pause);
		}
	}
}