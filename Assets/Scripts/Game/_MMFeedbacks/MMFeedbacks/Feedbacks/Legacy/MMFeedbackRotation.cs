...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbackRotation : MMFeedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (isActiveAndEnabled || _hostMMFeedbacks.AutoPlayOnEnable)
			{
				if ((Mode == Modes.Absolute) || (Mode == Modes.Additive))
				{
					...
					_coroutine = Timing.RunCoroutine(AnimateRotation(AnimateRotationTarget, Vector3.zero, FeedbackDuration, AnimateRotationX, AnimateRotationY, AnimateRotationZ, RemapCurveZero * intensityMultiplier, RemapCurveOne * intensityMultiplier));
				}
				else if (Mode == Modes.ToDestination)
				{
					...
					_coroutine = Timing.RunCoroutine(RotateToDestination());
				}
			}
		}
		...
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerator<float> RotateToDestination()
		{
		...
		/// <returns></returns>
		protected virtual IEnumerator<float> AnimateRotation(Transform targetTransform,
			Vector3 vector,
			float duration,
