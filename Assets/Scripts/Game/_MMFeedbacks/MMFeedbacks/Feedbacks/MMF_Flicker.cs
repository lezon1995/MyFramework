...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Flicker : MMF_Feedback
	{
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			_coroutines = new CoroutineHandle[MaterialIndexes.Length];
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			for (int i = 0; i < MaterialIndexes.Length; i++)
			{
				...
				_coroutines[i] = Timing.RunCoroutine(Flicker(BoundRenderer, i, _initialFlickerColors[i], FlickerColor, FlickerPeriod, FeedbackDuration));
			}
		}
		...
		public virtual IEnumerator<float> Flicker(Renderer renderer, int materialIndex, Color initialColor, Color flickerColor, float flickerSpeed, float flickerDuration)
		{
			if (renderer == null)
			{
				yield break;
			...
			while (FeedbackTime < flickerStop)
			{
				...
				yield return Timing.WaitUntilDone(WaitFor(flickerSpeed));
