...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_PlayerChain : MMF_Feedback
	{
		...
		/// </summary>
		/// <returns></returns>
		protected virtual IEnumerator<float> PlayChain()
		{
			IsPlaying = true;
			foreach (PlayerChainItem item in Players)
			{
				...
				if (item.Delay.x > 0)
				{
					yield return Timing.WaitUntilDone(WaitFor(item.Delay.x));
				}
				
				if (item.WaitUntilComplete) 
				{
					item.TargetPlayer.PlayFeedbacks();
					yield return Timing.WaitUntilDone(WaitFor(item.TargetPlayer.TotalDuration));
				} 
				else 
				{
					item.TargetPlayer.PlayFeedbacks();
				...
				if (item.Delay.y > 0)
				{
					yield return Timing.WaitUntilDone(WaitFor(item.Delay.y));
				}
			}
