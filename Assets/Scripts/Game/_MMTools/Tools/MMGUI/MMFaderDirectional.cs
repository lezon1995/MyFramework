...
namespace MoreMountains.Tools
{
	...
	public class MMFaderDirectional : MMMonoBehaviour, IEvent<MMFadeEvent>, IEvent<MMFadeInEvent>, IEvent<MMFadeOutEvent>, IEvent<MMFadeStopEvent>
	{
		/// the possible directions this fader can move in
		public enum Directions { TopToBottom, LeftToRight, RightToLeft, BottomToTop }

		[MMInspectorGroup("Identification")] 
		...
		[MMInspectorGroup("Directional Fader")]
        ...
        /// </summary>
        //protected virtual IEnumerator<float> Initialization()
        ...
        protected virtual IEnumerator<float> StartFading(bool fadingIn, float duration, MMTweenType curve, int id,
        ...
        /// </summary>
        /// <param name="e">Fade event.</param>
        public virtual void onEvent(MMFadeEvent e)
        {
            bool status = _active ? false : true;
            Timing.RunCoroutine(StartFading(status, e.Duration, e.Curve, e.ID,
                e.IgnoreTimeScale, e.WorldPosition));
        }
        ...
        /// </summary>
        /// <param name="e">Fade event.</param>
        public virtual void onEvent(MMFadeInEvent e)
        {
            Timing.RunCoroutine(StartFading(true, e.Duration, e.Curve, e.ID,
                e.IgnoreTimeScale, e.WorldPosition));
        }
        ...
        /// </summary>
        /// <param name="e">Fade event.</param>
        public virtual void onEvent(MMFadeOutEvent e)
        {
            Timing.RunCoroutine(StartFading(false, e.Duration, e.Curve, e.ID,
                e.IgnoreTimeScale, e.WorldPosition));
        }
        ...
        public virtual void onEvent(MMFadeStopEvent e)
        {
            if (e.ID == ID)
            {
                _fading = false;
        ...
        {
            this.addListener<MMFadeEvent>();
            this.addListener<MMFadeStopEvent>();
            this.addListener<MMFadeInEvent>();
            this.addListener<MMFadeOutEvent>();
        }
        ...
        {
            this.removeListener<MMFadeEvent>();
            this.removeListener<MMFadeStopEvent>();
            this.removeListener<MMFadeInEvent>();
            this.removeListener<MMFadeOutEvent>();
        }
    }
}