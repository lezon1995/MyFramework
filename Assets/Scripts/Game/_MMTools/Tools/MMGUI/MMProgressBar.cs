...
namespace MoreMountains.Tools
{
    ...
    public class MMProgressBar : MMMonoBehaviour
    {
        ...
        [MMInspectorGroup("Foreground Bar Settings")]
        ...
        [MMInspectorGroup("Delayed Bar Increasing")]
        [Tooltip("the delay before the delayed bar moves (in seconds)")]
        ...
        [MMInspectorGroup("Debug Read Only")]
        ...
        public virtual void UpdateBar(float currentValue, float minValue, float maxValue)
        {
            ...
            if (gameObject.activeInHierarchy)
                _coroutine = Timing.RunCoroutine(UpdateBarsCo());
            else
        ...
        /// </summary>
        protected virtual IEnumerator<float> UpdateBarsCo()
        {
        ...
        public virtual void Bump()
        {
            ...
            if (gameObject.activeInHierarchy)
            {
                Timing.RunCoroutine(BumpCoroutine(intensityMultiplier).CancelWith(gameObject));
            }
        ...
        protected virtual IEnumerator<float> BumpCoroutine(float intensityMultiplier)
        {
            float journey = 0f;
        ...
        public virtual void HideBar(float delay)
        {
            ...
            {
                Timing.RunCoroutine(HideBarCo(delay).CancelWith(gameObject));
            }
        }
        ...
        /// <param name="delay"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> HideBarCo(float delay)
        {
