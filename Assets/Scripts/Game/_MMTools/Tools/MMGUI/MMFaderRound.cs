...
namespace MoreMountains.Tools
{
	...
	public class MMFaderRound : MMMonoBehaviour, IEvent<MMFadeEvent>, IEvent<MMFadeInEvent>, IEvent<MMFadeOutEvent>, IEvent<MMFadeStopEvent>
	{
        ...
        {
            _currentTargetScale = (e.TargetAlpha == -1) ? MaskScale.y : e.TargetAlpha;
            StartFading(FaderMask.transform.localScale.x, _currentTargetScale, e.Duration, e.Curve, e.ID,
        ...
        public virtual void onEvent(MMFadeInEvent e)
        {
            if (e.Duration > 0)
            {
                StartFading(MaskScale.y, MaskScale.x, e.Duration, e.Curve, e.ID,
                    e.IgnoreTimeScale, e.WorldPosition);
            }
            else
            {
        ...
        public virtual void onEvent(MMFadeOutEvent e)
        {
            if (e.Duration > 0)
            {
                StartFading(MaskScale.x, MaskScale.y, e.Duration, e.Curve, e.ID,
                    e.IgnoreTimeScale, e.WorldPosition);
            }
            else
            {
