...
namespace MoreMountains.Tools
{
	...
	public abstract class MMCinemachineZone : MonoBehaviour
	{
        ...
        protected virtual void Start()
        {
            ...
            Timing.RunCoroutine(EnableCamera(CameraStartsActive, 1));
        }
		...
		/// <returns></returns>
		protected virtual IEnumerator<float> EnableCamera(bool state, int frames)
		{
			#if MM_CINEMACHINE || MM_CINEMACHINE3
			if (VirtualCamera == null)
			{
            ...
            if (frames > 0)
            {
                yield return Timing.WaitUntilDone(MMCoroutine.WaitForFrames(frames));
            }
        ...
        protected virtual void EnterZone()
        {
            ...
            Timing.RunCoroutine(EnableCamera(true, 0));
            foreach (GameObject go in ActivationList)
            {
                go.SetActive(true);
            }
        ...
        protected virtual void ExitZone()
        {
            ...
            Timing.RunCoroutine(EnableCamera(false, 0));
            foreach (GameObject go in ActivationList)
            {
                go.SetActive(false);
            }
