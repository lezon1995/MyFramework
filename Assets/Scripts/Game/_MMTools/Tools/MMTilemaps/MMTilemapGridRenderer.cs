...
namespace MoreMountains.Tools
{
    public class MMTilemapGridRenderer
    {
            ...
            MMTweenType slowRenderTweenType = null, MonoBehaviour slowRenderSupport = null)
        {
            ...
            switch (layer.FusionMode)
            {
                case MMTilemapGeneratorLayer.FusionModes.Combine:
                    grid = MMGridGenerator.InvertGrid(grid);
                    tile = null;
                    break;
                case MMTilemapGeneratorLayer.FusionModes.Subtract:
                    grid = MMGridGenerator.InvertGrid(grid);
                    break;
            }
            ...
            {
                Timing.RunCoroutine(SlowRenderGrid(grid, layer.TargetTilemap, tile, slowRenderDuration, slowRenderTweenType, 60));
            }
        ...
        /// <returns></returns>
        public static IEnumerator<float> SlowRenderGrid(int[,] grid, Tilemap tilemap, TileBase tile, float slowRenderDuration, MMTweenType slowRenderTweenType, int frameRate)
        {
