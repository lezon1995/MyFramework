    ...
    public static class MMDebug
    {
...
#if UNITY_EDITOR
        static bool _debugDrawEnabledSet;
#endif
        static bool _debugDrawEnabled;
        static bool _debugLogEnabled;
        static bool _debugLogEnabledSet;
        ...
        public static void DebugLogCommand(string command)
        {
            ...
            if (splitCommand.Length > 1)
            {
                ...
                commandInfo = methods.FirstOrDefault(m => m.GetParameters().Length > 0);
                ...
                var argumentAttribute = commandInfo.GetCustomAttributes<MMDebugLogCommandArgumentCountAttribute>(true).FirstOrDefault();
        ...
        public struct DebugLogItem
        {
            ...
            public DebugLogItem(object message, string color, int frame, float time, int timePrecision, bool displayFrameCount)
            {
                Message = message;
                Color = color;
                Frame = frame;
        ...
        public static string LogHistoryText
        {
            get
            {
                ...
                for (int i = 0; i < LogHistory.Count; i++)
                {
                    ...
                    {
                        log.Append("<color=#82d3f9>[" + LogHistory[i].Frame + "]</color> ");
                    }
        ...
        public static bool DebugDrawEnabled
        {
            get
            {
                ...
                {
                    _debugDrawEnabled = PlayerPrefs.GetInt(_editorPrefsDebugDraws) != 0;
                }
                else
                {
        ...
        public static void InstantiateOnScreenConsole(int fontSize = 25)
        {
            ...
            if (_console == null)
            {
                ...
                _console = Object.FindFirstObjectByType<MMDebugOnScreenConsole>();
            }

            if (_console == null)
            {
                // we instantiate the console
                GameObject loaded = Object.Instantiate(Resources.Load(_debugConsolePrefabPath) as GameObject);
        ...
        /// <param name="arrowHeadLength">Arrow head length.</param>
        /// <param name="arrowHeadAngle">Arrow head angle.</param>
        static void DrawArrowEnd(bool drawGizmos, Vector3 arrowEndPosition, Vector3 direction, Color color, float arrowHeadLength = 0.25f, float arrowHeadAngle = 40.0f)
        {
            if (!DebugDrawEnabled)
                return;
        ...
        public static void DrawGizmoRectangle(Vector2 center, Vector2 size, Color color)
        {
            ...
            var v3BotRight = new Vector3(center.x + size.x / 2, center.y - size.y / 2, 0);
            var v3BotLeft = new Vector3(center.x - size.x / 2, center.y - size.y / 2, 0);

            Gizmos.DrawLine(v3TopLeft, v3TopRight);
            Gizmos.DrawLine(v3TopRight, v3BotRight);
            Gizmos.DrawLine(v3BotRight, v3BotLeft);
            Gizmos.DrawLine(v3BotLeft, v3TopLeft);
        }
        ...
        public static void DrawGizmoRectangle(Vector2 center, Vector2 size, Matrix4x4 rotationMatrix, Color color)
        {
            ...
            var v3TopLeft = rotationMatrix * new Vector3(center.x - size.x / 2, center.y + size.y / 2, 0);
            var v3TopRight = rotationMatrix * new Vector3(center.x + size.x / 2, center.y + size.y / 2, 0);
            var v3BotRight = rotationMatrix * new Vector3(center.x + size.x / 2, center.y - size.y / 2, 0);
            var v3BotLeft = rotationMatrix * new Vector3(center.x - size.x / 2, center.y - size.y / 2, 0);
