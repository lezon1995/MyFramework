    ...
    public static class MMColors
    {
        ...
        public static Color GetColorAt(int index)
        {
            if (ColorDictionary == null)
                ColorDictionary = InitializeDictionary();

            index %= ColorDictionary.Count;
            return ColorDictionary[index];
        }

        public static Dictionary<int, Color> InitializeDictionary()
        {
            return ColorDictionary = new Dictionary<int, Color>
            {
                { 0, AliceBlue },
                { 1, AntiqueWhite },
                { 2, Aqua },
        ...
        public static Gradient SimpleGradient(Color32 startColor, Color32 endColor, float startAlpha = 1f, float endAlpha = 1f)
        {
            return new Gradient()
            {
                ...
                alphaKeys = new[]
                {
                    new GradientAlphaKey(startAlpha, 0),
