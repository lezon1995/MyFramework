...
namespace MoreMountains.Tools
{
    ...
    public static class MMMaths
    {
        ...
        public static float Remap(float x, float A, float B, float C, float D)
        {
            if (B - A == 0)
                return 0;
            return C + Mathf.Clamp01((x - A) / (B - A)) * (D - C);
        }

        public static Vector3 Remap(float x, float A, float B, Vector3 C, Vector3 D)
        {
            if (B - A == 0)
                return Vector3.zero;
            ...
            {
                x = Remap(x, A, B, C.x, D.x),
                y = Remap(x, A, B, C.y, D.y),
                z = Remap(x, A, B, C.z, D.z),
            };
