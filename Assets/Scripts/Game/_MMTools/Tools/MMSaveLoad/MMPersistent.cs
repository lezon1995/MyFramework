...
namespace MoreMountains.Tools
{
    ...
    public class MMPersistent : MMPersistentBase
    {
        ...
        public override void OnLoad(string data)
        {
            if (SavePosition)
            {
                transform.position = JsonUtility.FromJson<Data>(data).Position;
            }

            if (SaveLocalRotation)
            {
                transform.localRotation = JsonUtility.FromJson<Data>(data).LocalRotation;
            }

            if (SaveLocalScale)
            {
                transform.localScale = JsonUtility.FromJson<Data>(data).LocalScale;
            }

            if (SaveActiveState)
            {
                gameObject.SetActive(JsonUtility.FromJson<Data>(data).ActiveState);
            }
