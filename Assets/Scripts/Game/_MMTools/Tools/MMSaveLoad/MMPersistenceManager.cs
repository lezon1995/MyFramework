...
namespace MoreMountains.Tools
{
    ...
    /// </summary>
    public class MMPersistenceManager : MMPersistentSingleton<MMPersistenceManager>, IEvent<MMGameEvent>
    {
        [Header("Persistence")]
        ...
        {
            var saveData = MMSaveLoadManager.Load<MMPersistenceManagerData>( DetermineSaveName(), _saveFolderName);
            if (saveData is { SceneDatas: not null })
            {
                SceneDatas = new DictionaryStringSceneData();
        ...
        {
            if ((e.EventName == "Save") && ListenForSaveEvents)
            {
                Save();
            }

            if ((e.EventName == "Load") && ListenForLoadEvents)
            {
                Load();
            }

            if ((e.EventName == "SaveToMemory") && ListenForSaveToMemoryEvents)
            {
                SaveToMemory();
            }

            if ((e.EventName == "LoadFromMemory") && ListenForLoadFromMemoryEvents)
            {
                LoadFromMemory();
            }

            if ((e.EventName == "SaveToFile") && ListenForSaveToFileEvents)
            {
                SaveFromMemoryToFile();
            }

            if ((e.EventName == "LoadFromFile") && ListenForLoadFromFileEvents)
            {
                LoadFromFileToMemory();
            }
        }
