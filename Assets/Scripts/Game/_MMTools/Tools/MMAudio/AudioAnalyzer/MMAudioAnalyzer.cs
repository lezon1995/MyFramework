...
namespace MoreMountains.Tools
{
    ...
    public class MMAudioAnalyzer : MonoBehaviour
    {
        ...
        protected virtual IEnumerator<float> Analyze()
        {
            while (true)
            {
                switch (Mode)
                ...
                ComputeBandLevels();
                yield return Timing.WaitForSeconds(SampleInterval);
            }
        }
        ...
        protected virtual void OnBeat(Beat beat, float rawValue)
        {
            ...
            Timing.RunCoroutine(RemapBeat(beat));
        }

        protected virtual IEnumerator<float> RemapBeat(Beat beat)
        {
