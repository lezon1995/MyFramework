...
namespace MoreMountains.Tools
{
    ...
    public class MMPlaylist : MMMonoBehaviour
    {
        ...
        [MMInspectorGroup("Playlist Songs")]
        ...
        protected virtual void PlayFirstSong()
        {
            ...
            _coroutine = Timing.RunCoroutine(PlaySong(newIndex));
        }
        ...
        /// <returns></returns>
        protected virtual IEnumerator<float> PlaySong(int index)
        {
            ...
            if ((PlaylistState.Is(PlaylistStates.Playing)
                && (index >= 0 && index < Songs.Count)
                && !Songs[index].TargetAudioSource.isPlaying))
            {
                Timing.RunCoroutine(Fade(CurrentlyPlayingIndex,
            ...
            // initial delay
            yield return Timing.WaitForSeconds(Random.Range(Songs[index].InitialDelay.x, Songs[index].InitialDelay.y));
            ...
            Timing.RunCoroutine(Fade(index,
                Random.Range(Songs[index].CrossFadeDuration.x, Songs[index].CrossFadeDuration.y),
            ...
            while (Songs[index].TargetAudioSource.isPlaying || PlaylistState.Is(PlaylistStates.Paused) || _shouldResumeOnApplicationPause)
            {
                yield return Timing.WaitForOneFrame;
            }

            if (PlaylistState.Not(PlaylistStates.Playing))
            {
                yield break;
            }
            ...
            {
                _coroutine = Timing.RunCoroutine(PlaySong(PickNextIndex()));
            }
            else
            {
                if (Endless)
        ...
        /// <summary>
        /// Fades an audiosource in or out, optionally stopping it at the end
        /// </summary>
        ...
        /// <returns></returns>
        protected virtual IEnumerator<float> Fade(int index, float duration, float initialVolume, float endVolume, bool stopAtTheEnd)
        {
            if (index < 0 || index >= Songs.Count)
            {
                yield break;
        ...
        {
            _coroutine = Timing.RunCoroutine(PlaySong(songIndex));
        }
        ...
        protected virtual void OnPlayIndexEvent(int channel, int index)
        {
            ...
            _coroutine = Timing.RunCoroutine(PlaySong(index));
        }
        ...
        protected virtual void OnApplicationPause(bool pauseStatus)
        {
            ...
            if (pauseStatus && PlaylistState.Is(PlaylistStates.Playing))
            {
                Pause();
