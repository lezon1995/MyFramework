...
namespace MoreMountains.Tools
{
    ...
    public class MMSoundManager : MMPersistentSingleton<MMSoundManager>,
        IEvent<MMSoundManagerTrackEvent>,
        IEvent<MMSoundManagerEvent>,
        IEvent<MMSoundManagerSoundControlEvent>,
        IEvent<MMSoundManagerSoundFadeEvent>,
        IEvent<MMSoundManagerAllSoundsControlEvent>,
        IEvent<MMSoundManagerTrackFadeEvent>
    {
        /// <summary>
        ...
        [Tooltip("whether or not the pool can expand (create new audio-sources on demand). In a perfect world you'd want to avoid this, and have a sufficiently big pool, to avoid costly runtime creations.")]
        ...
        protected Dictionary<AudioSource, CoroutineHandle> _fadeInSoundCoroutines;
        protected Dictionary<AudioSource, CoroutineHandle> _fadeOutSoundCoroutines;
        protected Dictionary<MMSoundManagerTracks, CoroutineHandle> _fadeTrackCoroutines;
        ...
        {
            if (settingsSo && settingsSo.Settings.AutoLoad)
            {
        ...
        protected virtual void InitializeSoundManager()
        {
            ...
            _fadeInSoundCoroutines = new Dictionary<AudioSource, CoroutineHandle>();
            _fadeOutSoundCoroutines = new Dictionary<AudioSource, CoroutineHandle>();
            _fadeTrackCoroutines = new Dictionary<MMSoundManagerTracks, CoroutineHandle>();
        }
        ...
        )
        {
            ...
            if (!audioSource)
            {
                ...
                if (audioSource && !loop)
                {
                    ...
                    Timing.RunCoroutine(_pool.AutoDisableAudioSource(audioClip.length / Mathf.Abs(pitch), audioSource, audioClip, doNotAutoRecycleIfNotDonePlaying, playbackTime, playbackDuration));
                }
            }
            ...
            {
                var followTarget = audioSource.gameObject.MMGetComponentNoAlloc<MMFollowTarget>();
            ...
            if (settingsSo)
            {
                audioSource.outputAudioMixerGroup = mmSoundManagerTrack switch
                {
                    MMSoundManagerTracks.Master => settingsSo.MasterAudioMixerGroup,
                    MMSoundManagerTracks.Music => settingsSo.MusicAudioMixerGroup,
                    MMSoundManagerTracks.Sfx => settingsSo.SfxAudioMixerGroup,
                    MMSoundManagerTracks.UI => settingsSo.UIAudioMixerGroup,
                    _ => settingsSo.MasterAudioMixerGroup
                };
            }
            ...
            else if (soloAllTracks)
            {
                ...
                if (autoUnSoloOnEnd)
                {
                    Timing.RunCoroutine(MuteAllSoundsCoroutine(audioClip.length - playbackTime, false));
                }
            }
            ...
            // we return the audio-source reference
            return audioSource;
        }
        ...
        public virtual float GetTrackVolume(MMSoundManagerTracks track, bool mutedVolume)
        {
            return track switch
            {
                MMSoundManagerTracks.Master => mutedVolume ? settingsSo.Settings.MutedMasterVolume : settingsSo.Settings.MasterVolume,
                MMSoundManagerTracks.Music => mutedVolume ? settingsSo.Settings.MutedMusicVolume : settingsSo.Settings.MusicVolume,
                MMSoundManagerTracks.Sfx => mutedVolume ? settingsSo.Settings.MutedSfxVolume : settingsSo.Settings.SfxVolume,
                MMSoundManagerTracks.UI => mutedVolume ? settingsSo.Settings.MutedUIVolume : settingsSo.Settings.UIVolume,
                _ => 1f
            };
        }
        ...
        {
            var soundsPlaying = new List<MMSoundManagerSound>();
            foreach (var sound in _sounds)
            {
                if (sound.Track == track && sound.Source.isPlaying)
                {
        ...
        {
            CoroutineHandle coroutine = Timing.RunCoroutine(FadeTrackCoroutine(track, duration, initialVolume, finalVolume, tweenType));
        ...
        {
            CoroutineHandle coroutine = Timing.RunCoroutine(FadeCoroutine(source, duration, initialVolume, finalVolume, tweenType, freeAfterFade));
        ...
        {
            if (_fadeInSoundCoroutines.TryGetValue(source, out CoroutineHandle co))
            {
                return co != default;
            }
        ...
        {
            if (_fadeOutSoundCoroutines.TryGetValue(source, out CoroutineHandle co))
            {
                return co != default;
            }
        ...
        {
            CoroutineHandle outCoroutine;
            if (_fadeTrackCoroutines.TryGetValue(track, out outCoroutine))
            {
                Timing.KillCoroutines(outCoroutine);
        ...
        {
            CoroutineHandle outCoroutine;
            if (source && _fadeInSoundCoroutines.TryGetValue(source, out outCoroutine))
            {
                if (outCoroutine != default)
                {
                    Timing.KillCoroutines(outCoroutine);
            ...
            if (source && _fadeOutSoundCoroutines.TryGetValue(source, out outCoroutine))
            {
                if (outCoroutine != default)
                {
                    Timing.KillCoroutines(outCoroutine);
        ...
        /// <param name="tweenType"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> FadeTrackCoroutine(MMSoundManagerTracks track, float duration, float initialVolume, float finalVolume, MMTweenType tweenType)
        {
        ...
        /// <param name="tweenType"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> FadeCoroutine(AudioSource source, float duration, float initialVolume, float finalVolume, MMTweenType tweenType, bool freeAfterFade = false)
        {
            float startedAt = Time.unscaledTime;
            tweenType ??= new MMTweenType(MMTween.MMTweenCurve.EaseInOutQuartic);
            ...
            if (initialVolume < finalVolume)
            {
                _fadeInSoundCoroutines[source] = default;
            }
            else
            {
                _fadeOutSoundCoroutines[source] = default;
            }
        }
        ...
        {
            Timing.RunCoroutine(MuteSoundsOnTrackCoroutine(track, mute, delay));
        }
        ...
        {
            Timing.RunCoroutine(MuteAllSoundsCoroutine(0f, mute));
        }
        ...
        /// <param name="delay"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> MuteSoundsOnTrackCoroutine(MMSoundManagerTracks track, bool mute, float delay)
        {
            if (delay > 0)
            {
                yield return Timing.WaitUntilDone(MMCoroutine.WaitForUnscaled(delay));
            }
        ...
        /// <param name="mute"></param>
        /// <returns></returns>
        protected virtual IEnumerator<float> MuteAllSoundsCoroutine(float delay, bool mute = true)
        {
            if (delay > 0)
            {
                yield return Timing.WaitUntilDone(MMCoroutine.WaitForUnscaled(delay));
        ...
        public virtual void FreeAllSoundsButPersistent()
        {
            foreach (var sound in _sounds)
            {
                if (!sound.Persistent && sound.Source)
                {
                    FreeSound(sound.Source);
                }
            }
        ...
        public virtual void FreeAllLoopingSounds()
        {
            foreach (var sound in _sounds)
            {
                if (sound.Source.loop && sound.Source)
                {
                    FreeSound(sound.Source);
                }
            }
        ...
        public virtual void onEvent(MMSoundManagerTrackEvent e)
        {
            switch (e.TrackEventType)
            {
                ...
                case MMSoundManagerTrackEventTypes.SetVolumeTrack:
                    SetTrackVolume(e.Track, e.Volume);
                    break;
        ...
        public virtual void onEvent(MMSoundManagerEvent e)
        {
            switch (e.EventType)
            {
        ...
        public virtual void onEvent(MMSoundManagerSoundControlEvent e)
        {
            if (e.TargetSource == null)
                _tempAudioSource = FindByID(e.SoundID);
            else
                _tempAudioSource = e.TargetSource;

            if (_tempAudioSource)
            {
                switch (e.MMSoundManagerSoundControlEventType)
                {
        ...
        public virtual void onEvent(MMSoundManagerTrackFadeEvent e)
        {
            switch (e.Mode)
            {
                case MMSoundManagerTrackFadeEvent.Modes.PlayFade:
                    FadeTrack(e.Track, e.FadeDuration, settingsSo.GetTrackVolume(e.Track), e.FinalVolume, e.FadeTween);
                    break;
        ...
        public virtual void onEvent(MMSoundManagerSoundFadeEvent e)
        {
            _tempAudioSource = FindByID(e.SoundID);
            switch (e.Mode)
            {
        ...
        public virtual void onEvent(MMSoundManagerAllSoundsControlEvent e)
        {
            switch (e.EventType)
            {
        ...
        public virtual void OnMMSfxEvent(AudioClip clipToPlay, int id, float volume = 1f, float pitch = 1f, int priority = 128, AudioMixerGroup audioGroup = null)
        {
        ...
        protected virtual void OnEnable()
        {
            ...
            this.addListener<MMSoundManagerEvent>();
            this.addListener<MMSoundManagerTrackEvent>();
            this.addListener<MMSoundManagerSoundControlEvent>();
            this.addListener<MMSoundManagerTrackFadeEvent>();
            this.addListener<MMSoundManagerSoundFadeEvent>();
            this.addListener<MMSoundManagerAllSoundsControlEvent>();
        ...
        protected virtual void OnDisable()
        {
            if (_enabled)
            {
                ...
                this.removeListener<MMSoundManagerEvent>();
                this.removeListener<MMSoundManagerTrackEvent>();
                this.removeListener<MMSoundManagerSoundControlEvent>();
                this.removeListener<MMSoundManagerTrackFadeEvent>();
                this.removeListener<MMSoundManagerSoundFadeEvent>();
                this.removeListener<MMSoundManagerAllSoundsControlEvent>();
