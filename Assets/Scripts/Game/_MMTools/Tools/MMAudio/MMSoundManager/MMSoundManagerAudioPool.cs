...
namespace MoreMountains.Tools
{
    ...
    public class MMSoundManagerAudioPool
    {
        ...
        /// <returns></returns>
        public virtual IEnumerator<float> AutoDisableAudioSource(float duration, AudioSource source, AudioClip clip, bool doNotAutoRecycleIfNotDonePlaying, float playbackTime, float playbackDuration)
        {
            ...
            yield return Timing.WaitUntilDone(MMCoroutine.WaitForUnscaled(initialWait));
            if (source.clip != clip)
            {
                yield break;
            }
