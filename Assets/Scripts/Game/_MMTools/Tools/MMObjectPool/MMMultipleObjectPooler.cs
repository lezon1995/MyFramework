...
namespace MoreMountains.Tools
{
    ...
    public class MMMultipleObjectPooler : MMObjectPooler
    {
        ...
        [MMInformation("If you set CanPoolSameObjectTwice to false, the Pooler will try to prevent the same object from being pooled twice to avoid repetition. This will only affect random pooling methods, not ordered pooling.")]
        ...
        {
            if (string.IsNullOrEmpty(MutualizedPoolName))
                return "[MPooler] " + name;

            return "[MPooler] " + MutualizedPoolName;
        }
        ...
        public override void FillObjectPool()
        {
            ...
            switch (PoolingMethod)
            {
                case MMPoolingMethods.OriginalOrder:
                    var stillObjectsToPool = true;
                    // we store our pool sizes in a temp array so it doesn't impact the inspector
