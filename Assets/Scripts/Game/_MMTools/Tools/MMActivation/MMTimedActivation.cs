...
namespace MoreMountains.Tools
{
    ...
    public class MMTimedActivation : MonoBehaviour
    {
        ...
        {
            Timing.RunCoroutine(TimedActivationSequence());
        }

        /// <summary>
        /// Waits and triggers state change and events
        /// </summary>
        protected virtual IEnumerator<float> TimedActivationSequence()
        {
            if (DelayMode == DelayModes.Time)
            {
                yield return Timing.WaitForSeconds(TimeBeforeStateChange);
            }
            else
            {
                yield return Timing.WaitUntilDone(MMCoroutine.WaitForFrames(FrameCount));
            }
