...
public class LaunchSceneDownload : SceneProcedure
{
    ...
    public LaunchSceneDownload()
    {
        ...
        mInstance.setShowTipCallback(tip =>
        {
    ...
    // 这个函数的目的是确保最新的混淆密钥文件一定存在于PersistentPath中
    // 因为在启动热更时GameHotFixBase会固定从PersistentPath中加载密钥文件
    ...
    protected void launch()
    {
        HybridCLRSystem.launchHotFix(getAESKeyBytes(), getAESIVBytes(),
            ...
            (fileName, callback) =>
            {
                ...
                {
                    openFileAsync(availableReadPath(fileName), true, bytes =>
                    {
