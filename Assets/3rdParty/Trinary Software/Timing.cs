...
public class Timing : MonoBehaviour
{
    ...
    public static int KillCoroutines(ref CoroutineHandle handle)
    {
        var result = ActiveInstances[handle.Key] != null
            ? GetInstance(handle.Key).KillCoroutinesOnInstance(handle)
            : 0;
...
public static class MECExtensionMethods1
{
    ...
    {
        return gameObj == null ? Timing.RunCoroutine(coroutine) : Timing.RunCoroutineSingleton(coroutine, gameObj.GetInstanceID(), behaviorOnCollision);
    }
    ...
    public static CoroutineHandle RunCoroutineSingleton(this IEnumerator<float> coroutine, Segment segment, GameObject gameObj,
        SingletonBehavior behaviorOnCollision)
    {
        return gameObj == null ? Timing.RunCoroutine(coroutine, segment) : Timing.RunCoroutineSingleton(coroutine, segment, gameObj.GetInstanceID(), behaviorOnCollision);
    }
